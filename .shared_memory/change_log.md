...
- Diagnosed: The production frontend still serves the older route chunk whose visible toolbar passes `onSearchChange` to `setSearchQuery` only; its `index.html` hash differs from the tested local build and an rsync dry run shows the pending artifact replacement. No deployment occurred.
- Deployment: Not performed in this cycle; production artifacts and live mailbox behavior remain unchanged.
- Follow-up: Build the safe Rule Workbench preview seam using the shared search/rule predicate vocabulary before adding historical mailbox mutations.
