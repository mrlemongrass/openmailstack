...
- 🟡 CardDAV contact identity hardening: deployed commit `b575a57` makes new
  web/API/CSV/vCard-created contacts persist UUID-backed href/vCard identity,
  and duplicate scan results keep the UUID row as merge primary without
  rewriting legacy `contact-<id>` rows. Focused, complete local, guarded
  release, artifact, and staging checks pass; a physical macOS edit/merge retry
  remains. Do not replace this with name/email-based PUT merging, which would
  collapse legitimate contacts and violate href-based CardDAV semantics.
...
- Existing-mail rule Apply must stay bound to the previewed complete saved-rule document, canonical selected-rule list, read state, subfolder choice, and the server-authored ordered snapshot of every source folder's UIDVALIDITY and UID ceiling. Reject a client-supplied initial scope snapshot, cap folder expansion, use LIST-STATUS instead of sequential STATUS calls, and preflight every snapshotted UIDVALIDITY before the first mutation. Resolve explicit selection only from enabled saved rules in saved order and reject malformed, duplicate, unknown, disabled, or changed selectors before mailbox work. If legacy rule IDs/names collide, use document-wide deterministic positional selectors so each rule remains independently selectable and a reorder invalidates the preview; omitted `ruleIds` remains the backward-compatible all-enabled path. Continued copies rely on `mail_rule_copy_ledger`: use a rule-revision-independent action key from mailbox identity/current source folder/source UID/destination, allow only one pending action per source UID, reserve only the destination group about to be copied, mark confirmed completion, skip completed actions on replay, and resolve only the exact pending action keys shown to the owner from that actual source folder. Pending copies must block even a later Move-only edit. Do not age out unresolved rows, make Apply browser-cancellable, or bypass that durable idempotency contract.
- Existing-mail match details must remain preview-only and metadata-only: never return message bodies or accept `includeMatchDetails` during Apply. Keep the first rendered page bounded to 20, load later pages only from the exact preview revision, canonical selection, read state, and server-authored scope snapshot, and resume after the last reviewed folder/UID cursor. UID ceilings exclude new arrivals but cannot prevent deletion or read-state drift; if later detail counts become short, excessive, stalled, or otherwise inconsistent, stop pagination and require a new Preview instead of presenting a false range. Do not cache an unbounded full mailbox match list in the browser.
