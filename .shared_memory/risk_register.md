...
- Scheduler Phase 2 unlisted, private, single-use, and one-off event links are live and tested. Private links use hash-only 256-bit tokens, fragment-to-header browser transport, tab-only storage, rotation, bounded optional expiry, revocation, generic failures, and automatic revocation when leaving Private visibility. Single-use booking consumption is row-locked, transactional, audited, and idempotently replayable. One-off links are bounded to 14 windows within 62 days, force single-use, and reuse normal conflict/capacity enforcement; custom booking questions are the next personal-scheduling slice.
