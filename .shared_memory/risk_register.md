...
- Scheduler Phase 2 private-link and booking-question slices are live and tested. Private links use hash-only 256-bit tokens, fragment-to-header transport, rotation/expiry/revocation, and transactional single-use consumption. One-off links are bounded to 14 windows within 62 days. Booking questions are bounded to ten required/optional short, long, or dropdown fields; answers are immutable, validated, owner-visible, output-escaped, and excluded from audit/outbox/calendar/public-capability surfaces. Optional host confirmation is the next personal-scheduling slice.
