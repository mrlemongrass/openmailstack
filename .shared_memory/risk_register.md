...
- OMS Scheduler Phase 1 has one enabled live mailbox and its public profile is available on both configured hostnames, but no event type exists yet. Before release, test both disabled and enabled fresh installs, then verify real SMTP/ICS notification delivery and booking/reschedule/cancel propagation through OMS Calendar, CalDAV, and ActiveSync clients. The live profile/service/route smoke is not a substitute for this workflow.
