...
- Scheduler Phase 2 private-link, booking-question, and optional host-confirmation slices are live and tested. Private links use hash-only 256-bit tokens, fragment-to-header transport, rotation/expiry/revocation, and transactional single-use consumption. One-off links are bounded to 14 windows within 62 days. Booking questions are bounded to ten required/optional short, long, or dropdown fields; answers are immutable, validated, owner-visible, output-escaped, and excluded from audit/outbox/calendar/public-capability surfaces. Requested bookings deliberately count against the existing `confirmed_seats` inventory counter as reserved capacity before Calendar projection; preserve this semantic if the inventory schema is renamed or expanded. Owner-configurable cancellation/reschedule cutoffs and reason collection are the next personal-scheduling slice.
