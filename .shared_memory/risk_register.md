...
- OMS Scheduler is planned but not implemented. Its public unauthenticated surface, tenant model, slot concurrency, workflow worker, provider secret vault, OAuth, payments, webhooks, routing, and external calendar reconciliation are all new security and reliability boundaries. Do not begin with booking-page UI alone; first prove DST-safe availability, transactional slot holds, tenant ownership, idempotency, and an outbox/worker design as specified in `docs/product/scheduler.md`.
...
- Public Scheduler paths use the mailbox local part without its domain. Multi-domain installations can contain the same local part more than once, and local parts can collide with application routes or contain unsuitable URL characters. Enforce a globally unique normalized handle, reserve static route names, and require an admin-assigned alternate handle before enabling an ambiguous mailbox.
- Scheduler must serve the same path on configured webmail hostname aliases, but absolute links must not trust arbitrary request `Host` values. Keep an explicit preferred public base URL and alias allowlist, validate Nginx/DNS/TLS coverage, and use the preferred base for email, embeds, canonical metadata, OAuth callbacks, and webhook configuration.
...
- Frontend lint now exits 0, but warning debt remains broad: staged `any` typing plus React compiler-style hook migration warnings. The current frontend main build chunk is also above the documented 500 kB target. Treat typed API/domain cleanup and the next code-splitting pass as release-quality work before a formal release.
