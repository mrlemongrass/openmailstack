...
- 🟡 Reach personal parity with advanced limits, overrides, private/single-use links, seats, recurring bookings, meeting polls, embeds, and migration/import. Phase 2 now includes the complete private-link slice, deployed required/optional booking questions, optional host confirmation, and immutable cancellation/reschedule cutoffs with private reason collection. Per-event active booking limits keyed by normalized guest email, with a safe reschedule offer, are the next bounded personal-scheduling slice.
