...
- 🟡 Reach personal parity with advanced limits, overrides, private/single-use links, seats, recurring bookings, meeting polls, embeds, and migration/import. Phase 2 now includes the complete private-link slice, booking questions, host confirmation, immutable guest action policies, serialized active-booking limits, guest email/domain rules, optional email verification, named additional attendees, and transactional seat counts. A capacity-aware waitlist is the next bounded personal-scheduling slice.
