...
- 🟡 Reach personal parity with advanced limits, overrides, private/single-use links, seats, recurring bookings, meeting polls, embeds, and migration/import. Phase 2 now includes the complete deployed private-link slice: unlisted events, hash-only private tokens with rotation/expiry/revocation, fragment-to-header browser handling, atomic single-use consumption, and owner-selected one-off availability. Custom booking questions are the next bounded personal-scheduling slice.
