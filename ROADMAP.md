...
- 🟡 Webmail search interaction and correctness are repaired locally (2026-07-21): 300 ms debounce plus immediate Enter submission, immediate clear/reset, explicit field plus current-folder/all-mail controls, complete request serialization, UIDVALIDITY-aware index/IMAP reconciliation, current flag and stale move/delete cleanup, folder-safe actions, bounded attachment verification, visible partial results, and globally newest all-folder ranking are regression-covered. Production rollout remains pending; the current live bundle still wires the visible input only to local query state.
