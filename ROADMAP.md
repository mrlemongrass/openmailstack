...
- 🟡 **Phases 1 and 2 repository scope complete; live hardening deployment pending**: installation/entitlements, event types, availability, native booking, private/one-off links, booking integrity, waitlists, DST-safe recurring series, meeting polls, delegation/outcomes, public embeds/customization/attribution, and guided import/export are live through migration `023`. Repository hardening revalidated migrations `001`-`023` twice and all 90 backend tests without skips, including policy-change waitlist promotion, but those backend artifacts still need guarded live synchronization. A real create/reschedule/cancel cycle passed SMTP, outbox, Calendar projection, tombstone, capacity-release, and public-slot restoration checks. Physical CalDAV/ActiveSync observation remains pending; clean-VM validation is deferred until a second development Linux server is available.
