...
Evidence: `webmail-backend/src/scheduler/`, versioned migrations `001` through `011`, `functions/12_scheduler.sh`, `webmail-frontend/src/scheduler/`, backend tests, static integration guards, disposable MariaDB lifecycle/concurrency tests, and the live 2026-07-12 deployment. Live Nginx serves Scheduler on both configured hostnames, the backend/frontend match the tested build, real booking/private-link lifecycles pass, one-off filtering, booking-question validation, host confirmation, and guest action-policy contracts pass live, and the staging smoke suite passes. Remaining validation is physical CalDAV/ActiveSync observation and clean-VM work after a second development Linux server is available.
