...
Evidence: `webmail-backend/src/scheduler/`, versioned migrations `001` through `005`, `functions/12_scheduler.sh`, `webmail-frontend/src/scheduler/`, backend tests, static integration guards, disposable MariaDB lifecycle tests, and the live 2026-07-12 deployment. Live Nginx serves Scheduler on both configured hostnames, the backend/frontend match the tested build, a real create/reschedule/cancel cycle passed SMTP/outbox/Calendar/tombstone/capacity checks, unlisted directory isolation passed, and the staging smoke suite passes. Remaining validation is physical CalDAV/ActiveSync observation plus clean-VM work after a second development Linux server is available.
