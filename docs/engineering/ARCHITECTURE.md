...
Evidence: `webmail-backend/src/scheduler/`, versioned migrations `001` through `010`, `functions/12_scheduler.sh`, `webmail-frontend/src/scheduler/`, backend tests, static integration guards, disposable MariaDB lifecycle/concurrency tests, and the live 2026-07-12 deployment. Live Nginx serves Scheduler on both configured hostnames, the backend/frontend match the tested build, real booking/private-link lifecycles pass, one-off filtering, booking-question validation, and the host-confirmation public contract pass live, and the staging smoke suite passes. Remaining validation is physical CalDAV/ActiveSync observation and clean-VM work after a second development Linux server is available.
