...
Evidence: `webmail-backend/src/scheduler/`, versioned migrations `001` through `007`, `functions/12_scheduler.sh`, `webmail-frontend/src/scheduler/`, backend tests, static integration guards, disposable MariaDB lifecycle/concurrency tests, and the live 2026-07-12 deployment. Live Nginx serves Scheduler on both configured hostnames, the backend/frontend match the tested build, real booking and private-link lifecycles pass, single-use schema/state and failed-booking preservation pass live, and the staging smoke suite passes. Remaining validation is one-off customized availability, physical CalDAV/ActiveSync observation, and clean-VM work after a second development Linux server is available.
