...
| Contacts creation and detail | The reused 85%-height setup modal clipped the Save action on mobile, its fields and close icon lacked accessible names, and outside clicks discarded the draft. Opening a saved contact then called nonexistent mail/calendar activity tables and emitted duplicate 500s. | Contacts now uses a labelled opaque editor with explicit dismissal, connected field labels, a scrollable body, persistent Cancel/Save footer, single-column mobile names, and full-screen mobile geometry. Activity uses the existing mail search index plus owner-scoped Calendar storage. | At 390×844 and 1440×900, authenticated local Playwright verified the editor geometry, focus, outside-click, Escape, and labelled controls. A uniquely named temporary contact was created, opened, moved to trash, and confirmed absent; the activity HTTP regression proves the route avoids the nonexistent tables. |
