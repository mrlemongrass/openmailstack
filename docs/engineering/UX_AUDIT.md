...
| Contacts / mobile | The virtualized grid forced three columns after the sidebar disappeared, clipping cards and removing the only New Contact action. | Mobile now renders one full-width card per virtual row, measures each rendered row rather than trusting the initial estimate, hides the redundant grid/list switch, and exposes a primary New Contact action that opens the editor. | At 390×844, authenticated Playwright first measured 358 px cards and opened Create Contact without saving. Review remediation then rendered 60 long-name/long-organization fixtures: every row measured 122 px, the total virtual height reached 7320 px, no rows overlapped, and document width stayed 390 px. |
...
| Scheduler event-type editor / mobile | The five-section settings strip measured 468 px inside a 318 px viewport, clipping Public and completely hiding Advanced. A backdrop click also discarded an in-progress event-type draft. | Mobile now uses the suite's labelled opaque section picker while desktop retains the five-tab strip. The backdrop no longer dismisses the editor; Close, Cancel, and Escape remain intentional exit paths. | Authenticated live Playwright after deployment at 390×844 reached all five sections with zero document overflow, preserved a typed title after an outside click, and dismissed with Escape. At 1440×900, all five desktop tabs remained visible. No event type was saved. |
| Login / dark | The dark tenant logo nearly disappeared against the dark translucent sign-in card. | The unchanged uploaded mark now sits on a restrained light logo surface with a subtle shadow; Admin preview uses the same custom-logo treatment and the same original Mail-tile fallback when no login logo exists. | Playwright at 1440×900 confirmed the custom mark is legible and the 390×844 surface has no overflow. A blank-login-logo fixture with a separate app icon rendered the Mail fallback in Admin preview, with no logo surface or image, matching the real sign-in branch. |
...
| Admin / mobile | Opening the Admin drawer placed its 14 links on a translucent surface directly over dashboard text, and the apparent menu toggle was actually underneath the drawer. | The mobile drawer now uses the opaque theme surface, dims/inerts the page behind it, exposes a dedicated close button, hides links while closed, traps focus while open, closes on Escape, and restores focus to Menu. | At 390×844, authenticated Playwright computed `rgb(17, 24, 39)`, 14 labelled links, and no overflow. Focus moved to Close Admin menu; Shift+Tab wrapped to Spam and Tab returned to Close; Escape hid the drawer and restored Menu focus. At 1440 px the static sidebar remained unchanged. |
| Sync / mobile | The panel retained 40 px desktop padding, leaving 268 px setup cards where long CalDAV/CardDAV URLs ran underneath indistinguishable copy buttons; the four-part status header was cramped. | Mobile now uses 12 px page and 16 px panel gutters, wraps the header and endpoint text, and names every copy action for its protocol. | At 390×844, authenticated Playwright measured a 366 px panel and five 332 px rows; every endpoint ended at x=298 before its copy action at x=310, all five copy names were distinct, and there was no overflow. At 1440 px, the 640 px panel, 40 px padding, and 558 px rows remained unchanged. |
| Compose / reply / mobile | Compose's callback ref could steal focus during rerenders, the translucent 85%-height card competed with the app behind it, duplicate contact emails produced duplicate suggestions and React-key errors, and inline-reply actions wrapped into narrow labels. | Compose now uses stable recipient focus, labelled modal semantics, an opaque full-screen mobile sheet, grouped status/actions, opaque popovers, and case-insensitive email deduplication. Inline reply gives Send a full-width primary row with two readable secondary actions. | Authenticated local Playwright through the real backend at 390×844 typed continuously into To, showed one `localtest@housevo.us` suggestion, reported zero console errors after a clean reload, and captured full-screen compose plus readable inline-reply layouts without sending mail. |
| Calendar event creation | The event editor used a translucent surface, discarded drafts on an accidental outside click, exposed several unnamed controls, squeezed timed fields on mobile, and repeated duplicate contact suggestions. | The editor is now a labelled, keyboard-contained opaque dialog with explicit dismissal, accessible control names, stacked mobile time fields, a persistent safe-area action footer, and the suite's shared case-insensitive suggestion deduplication. | Authenticated local Playwright through the real backend verified the all-day, timed, expanded-options, suggestion, outside-click, Escape, desktop, and 390×844 mobile states with zero console errors. A uniquely named temporary event was created, reopened, deleted, and confirmed absent. |
| Contacts creation and detail | The reused 85%-height setup modal clipped the Save action on mobile, its fields and close icon lacked accessible names, and outside clicks discarded the draft. Opening a saved contact then called nonexistent mail/calendar activity tables and emitted duplicate 500s. | Contacts now uses a labelled opaque editor with explicit dismissal, connected field labels, a scrollable body, persistent Cancel/Save footer, single-column mobile names, and full-screen mobile geometry. Activity uses the existing mail search index plus owner-scoped Calendar storage with exact normalized-address matching. | At 390×844 and 1440×900, authenticated local Playwright verified the editor geometry, focus, outside-click, Escape, and labelled controls. A uniquely named temporary contact was created, opened, then permanently deleted and confirmed absent; HTTP regressions prove the activity route avoids nonexistent tables, does not confuse prefix addresses, and does not truncate event candidates before future-occurrence selection. Post-deploy live verification returned `200` from the contact activity endpoint. |

### Prioritized open findings

No unresolved findings remain from this bounded 2026-07-29 audit backlog. Continue auditing one authenticated surface and workflow at a time; absence from this list is not a claim that the suite has no other UX defects.
