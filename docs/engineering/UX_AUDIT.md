...
| Admin / mobile | Opening the Admin drawer placed its 14 links on a translucent surface directly over dashboard text, and the apparent menu toggle was actually underneath the drawer. | The mobile drawer now uses the opaque theme surface, dims the page behind it, exposes a dedicated close button, and removes its links from visibility/focus while closed. | At 390×844, authenticated Playwright computed `rgb(17, 24, 39)`, a visible 240 px drawer, 14 labelled links, and no overflow. Close returned `visibility: hidden`; selecting Branding closed the drawer and reached `/admin/branding`. At 1440 px the static 240 px sidebar remained unchanged. |
