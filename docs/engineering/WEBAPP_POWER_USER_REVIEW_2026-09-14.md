...
- Full frontend suite: 276 passed. Full backend suite: 1,013 passed, seven skipped, no failures. Subsequent affected checks and release evidence are recorded in WORKLOG.
...
- New Junk entries are enforced only when the managed rule is successfully compiled/saved/activated. Server security checks still apply. No blanket whitelist, global domain ban, automatic deletion policy or retrospective sweep of other messages is introduced.
