...
- [x] Owners can cap future requested/confirmed bookings per event and normalized guest email. A per-event/email mutex serializes concurrent attempts, successful idempotency replays bypass the cap safely, cancellation releases the allowance, and the conflict response points guests back to the secure management/reschedule link in their booking email.
- [x] Owners can allow or deny up to 100 exact guest emails or `@domain` rules. Denials win, non-empty allow lists become exclusive, the same policy covers the booker and named attendees before capacity is acquired, and public event responses never expose the configured rule lists.
- [x] Owners can require a 15-minute email code before booking. Challenges bind to event plus normalized email, store only a hash, allow five attempts, consume under the booking transaction, reserve no capacity when requested, and redact delivered or dead-lettered verification payloads.
- [x] Owners can allow up to 20 named additional attendees within event capacity. Attendees are bounded, normalized, unique, different from the booker, eligibility-checked, stored with the booking snapshot, rendered for the owner, projected into iCalendar, and notified only after confirmation without receiving the booker's management links.
- [x] Owners can label holiday and out-of-office date ranges on reusable availability. Local-date exclusions block inherited slots across UTC boundaries and remain private owner context.
- [x] Capacity-aware waitlists accept only valid full future slots, preserve guest/verification/seat policy, and promote the oldest eligible fitting party after cancel, rejection, or reschedule releases capacity.
- [x] Guests can request up to twelve weekly occurrences. Every occurrence is conflict-checked, host-local time survives DST, idempotent series requests serialize, and failed series compensate without partial notification delivery.
- [x] Owners can offer two to ten available poll options through a one-time public link, require the event's email verification policy, collect replaceable votes, and finalize one option into a policy-checked owner-delegated booking.
- [x] Owners can create audited bookings on behalf of guests and mark ended confirmed bookings completed or no-show. Booking detail identifies the delegated actor, attribution, and series position.
- [x] Public events support accent color, introduction, HTTPS privacy/terms links, ten locale/date-format choices, and an optional locked IANA timezone. Query prefill is allowlisted to name/email/notes/slot and UTM fields are normalized into private booking attribution.
- [x] Owner tools generate inline, popup, floating-button, and email-slot snippets. Inline pages remove suite chrome and confirmations emit a privacy-safe `oms.scheduler.booking.confirmed` parent event.
- [x] Owners can export configuration JSON and spreadsheet-safe booking CSV, then import OMS, Calendly, or Cal.com JSON into inactive unlisted drafts with item-level results. Imported content never publishes automatically.
