...
Follow-up issue: Harden the raw ManageSieve response parser so literal script content and TCP chunk boundaries cannot be confused with protocol status lines; preserve the existing rule-order, stop-processing, preview-revision, and copy-ledger contracts.
```
