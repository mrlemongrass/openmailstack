...
- Pass with warnings: `rtk npm --prefix webmail-frontend run lint` exits 0. The remaining warnings are the staged `any` typing and React compiler-style hook migration backlog.
- Pass with warning: `rtk npm --prefix webmail-frontend run build` completed, but emitted a chunk-size warning; the current main chunk was `606.75 kB`, above the 500 kB target.
