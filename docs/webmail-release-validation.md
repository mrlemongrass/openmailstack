...
| TLS names | Pass for active names; autoconfig pending | Public IMAP, local HTTPS, and SMTP submission hostname verification pass with the Let's Encrypt certificate covering `mail.housevo.us`, `autodiscover.housevo.us`, and `webmail.housevo.us`; it expires 2026-10-05. The newly routed `autoconfig.housevo.us` name is not yet in the certificate and remains a setup gate. |
| Public DNS | Pass for active names; autoconfig pending | External DNS lookup confirmed `housevo.us` MX points to `mail.housevo.us`; `mail`, `autodiscover`, and `webmail` resolve to the public server address. `autoconfig.housevo.us` currently has no A/AAAA/CNAME answer. |
...
| Mozilla Autoconfiguration | Server pass; public hostname pending | The provider and well-known routes return `%EMAILADDRESS%`, IMAP 993 SSL, and SMTP 587 STARTTLS on the live backend. Forced `autoconfig.housevo.us` host routing returns 200, but public DNS and certificate SAN coverage are still required before Thunderbird automatic setup can be rerun. |
