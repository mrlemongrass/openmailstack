...
| Named Apple clients | Pass | macOS 26.5.2 Mail/CalDAV/CardDAV and iOS 26.5.2 ActiveSync Mail/Calendar/Contacts pass. The macOS Contacts Default Account picker remains a client UI issue despite a physically proven targeted CardDAV write/delete lifecycle. |
...
Last checked: 2026-08-03 against `mail.housevo.us`.
...
| TLS names | Pass | Public IMAP, HTTPS, and SMTP submission hostname verification pass with the Let's Encrypt certificate covering `autoconfig.housevo.us`, `autodiscover.housevo.us`, `mail.housevo.us`, and `webmail.housevo.us`; it expires 2026-11-01. Strict public TLS verification for `autoconfig.housevo.us` returns `Verify return code: 0 (ok)`. |
| Public DNS | Pass | External DNS lookup confirmed `housevo.us` MX points to `mail.housevo.us`; `mail`, `autodiscover`, and `webmail` resolve to the public server address, and `autoconfig.housevo.us` is a CNAME to `mail.housevo.us`. |
...
| Mozilla Autoconfiguration | Pass | Both live routes return `%EMAILADDRESS%`, IMAP 993 SSL, and SMTP 587 STARTTLS. Thunderbird 140.12.0esr and Thunderbird Android 21.1 fetched the public provider route, displayed the secure settings, and authenticated without manual correction. Redacted Nginx access evidence records the desktop Thunderbird user agent and Android `okhttp/5.3.2`; no query value is reflected in the XML. |
...
| Server-side Sieve delivery | Pass after userdb and parser hardening | Dovecot's SQL userdb returns an absolute `home` and `mail_path`, and a disposable LMTP delivery proved the active personal script can `fileinto` without an Inbox copy. The ManageSieve client now uses byte-exact literal framing, CRLF-complete terminal status, bounded 10 MiB literals/response overhead, and explicit mid-response EOF rejection. After guarded deployment, three delegated read-only `GETSCRIPT webmail` calls consistently returned 84,349 bytes; no filter or mailbox state was changed. |
...
| Real devices | Pass with documented macOS picker issue | Physical iPhone Exchange mail send/receive, complete Inbox catch-up, no-change polling, full message-body rendering, Sent copy, picture attachment, calendar create/edit/delete, and contact create/edit/delete pass for `thang@housevo.us`. The owner confirmed iOS Exchange, iOS IMAP, and macOS Mail all look correct after the MIME and certificate repairs. macOS 26.5.2 explicitly created and deleted a disposable contact in the native Housevo CardDAV container; live logs recorded the matching PUT/DELETE and storage cleanup. HouseVo still remains absent from Default Account, isolated as a macOS picker issue rather than server writability. |
...
| --- | --- | --- | --- | --- | --- |
| iPhone Exchange | Send, discovery, Sent copy, picture attachment, complete Inbox catch-up, stable no-change polling, and body rendering passed for `thang@housevo.us`. A July 14 strict-TLS regression and July 31 inherited-preview MIME truncation were fixed and physically revalidated; the owner reported iOS Exchange working normally alongside iOS IMAP. | Create, edit, and delete passed. iPhone edit to 18:45 Baghdad reached server storage, macOS Calendar after Command-R, and the web calendar. Delete removed the event from iOS and the web calendar; after the namespace-prefixed CalDAV sync fix and one approved Personal calendar sync-token bump from `1363` to `1364`, macOS Calendar also removed the event. | Create, edit, and delete passed. Delete removed `OMS iPhone Contact Test` from iPhone and the web Contacts app, and live storage has the matching contact tombstone. macOS Contacts retained the deleted contact and showed macOS-only duplicate contacts until the macOS CardDAV account was removed, Contacts was closed/reopened, and the account was re-added. Server checks found the test contact only as a soft-deleted row/tombstone and no duplicate active DAV UID rows. | Pass | Autodiscover should avoid manual server URL entry; if prompted, use `mail.housevo.us`. Initial send attempt returned "message was rejected by the server" because iOS placed raw MIME bytes under a decoded fallback node while `Mime` contained a client id. Server-side fix deployed 2026-07-11; physical retry passed at 04:33 Baghdad time and Gmail received it at 04:34. A later loopback/certificate-hostname mismatch was fixed in `e8caa78b`; physical retry on 2026-07-14 completed ActiveSync SMTP, Sent append, and remote gateway acceptance. Picture attachment passed at 04:41 Baghdad time. On July 31, live WBXML evidence showed iOS requested `BodyPreference Type=4` with no `TruncationSize`; the server incorrectly reused the stored 500-byte list-preview limit and returned MIME headers without a body boundary. The deployed fix uses the bounded complete-content ceiling for that request shape, while preserving explicit truncation and no-new-preference behavior. The physical partnership now holds 6,243 saved Inbox identities with a nonzero checkpoint/MODSEQ, zero pending commands, and `MoreAvailable=false`. Calendar create stored as `DTSTART:20260711T150000Z`; final edit retry stored as `DTSTART:20260711T154500Z`. Web calendar realtime refresh, CalDAV namespace-prefixed sync tombstone fixes, Contacts list UX/search/multi-phone fixes, ActiveSync/CardDAV contact tombstone delta sync, and CardDAV depth-1 PROPFIND tombstone compatibility with legacy href aliases are deployed. |
| macOS Mail/Calendar/Contacts | After the IMAP certificate repair, the owner confirmed macOS Mail loaded and worked normally without the certificate error. | CalDAV single-event create/edit/delete passed on macOS 26.5.2; a four-occurrence New York weekly series crossed DST with the correct Baghdad shift; edit-one/delete-one recurrence exceptions and reminders projected correctly through OMS Web and iOS. | HouseVo authenticates and syncs. An explicitly targeted Contacts-framework contact created in Housevo, produced a live CardDAV PUT, verified its destination, then deleted through a live CardDAV DELETE with clean tombstone state. HouseVo remains absent from Default Account despite proven writability. | Mail/Calendar/CardDAV pass; Contacts picker issue open | Keep mail, calendar, and contacts as separate IMAP/CalDAV/CardDAV accounts. The Default Account omission is isolated to the macOS picker; do not broaden DAV privileges or modify private Apple account databases. |
| Android mail plus DAVx5 | Thunderbird Android 21.0 on Android 11 (API 30) connected manually to IMAP 993 with SSL/TLS and SMTP 587 with STARTTLS. `OMS Android Thunderbird Matrix 20260730T1710Z` appeared in both the Android Inbox and Sent views and as server UIDs 30 and 9 before exact cleanup. A fresh Thunderbird Android 21.1 run then reported `Configuration found`, displayed `mail.housevo.us:993` SSL/TLS and `mail.housevo.us:587` StartTLS with full-address usernames, authenticated, and reached Inbox without a manual edit or mail mutation. | DAVx5 4.5.18-ose discovered the Personal calendar from `/.well-known/caldav`. Etar 1.0.56 created, edited, and deleted the event; server event 1457 retained UID `b5970849-2731-417c-bbd8-4213d72ab9db` through the edit and produced the expected tombstone on delete. | A separate DAVx5 account using `/.well-known/carddav` discovered Personal Contacts. Android Contacts created, edited, and deleted contact 1510 with stable DAV UID `52bc33ce-50ee-471d-b5e4-b8cfcddf8d60`; the delete produced sync token 56 and the expected tombstone. | Pass (Android emulator) | Run on Android Emulator 37.1.11 in the Debian 13.6 `dev2-debian` LXC. The host has no `/dev/kvm`, so the completed API 30 runs used CPU emulation after an API 36 image proved unstable. The bundled API 30 Calendar viewer lacked its declared event-editor class, so the verified F-Droid Etar build exercised the native Android Calendar provider. Thunderbird Android 21.1 was downloaded from the official stable GitHub release and matched published SHA-256 `699a48aeaa4ffca88ad868655e356995f943933fafe5f1d7773f803b866fc717`. All active test mail/event/contact state, profiles, AVDs, APKs, screenshots, and cached credentials were removed. |
| Thunderbird desktop | Thunderbird 140.12.0esr on Debian 13.6 sent `OMS Thunderbird Matrix 20260730T1520Z` through SMTP submission and received it over IMAP; the message appeared in Inbox and Sent as server UIDs 29 and 8 before exact cleanup. A fresh isolated-profile run then reported `Configuration found at email provider`, selected `mail.housevo.us` IMAP SSL/TLS and SMTP STARTTLS with the full address, authenticated, and displayed `Account successfully created`; its profile recorded ports 993 and 587 with the expected TLS modes. | `/.well-known/caldav` discovered the calendars. Create/edit/delete in Personal retained event UID `6777c423-ab92-4a87-8517-e8e525a5f853` and produced the expected tombstone. | `/.well-known/carddav` discovered Personal Contacts. Create/edit/delete retained DAV UID `d48307b8-6570-4f48-9af0-0a9d3e60e82d` and produced the expected tombstone. | Pass | The earlier automatic guess used short username `localtest` and SMTP without encryption. Public Mozilla Autoconfiguration now supplies the safe full-address/993 SSL-TLS/587 STARTTLS settings without manual correction. The closure run did not send or mutate mail, and all disposable profile, screenshot, and credential state was removed. |
...
Follow-up resolution (2026-08-03): The raw ManageSieve response parser now uses byte-exact literal framing, complete CRLF terminal lines, explicit EOF rejection, and bounded inbound responses. The existing rule-order, stop-processing, preview-revision, and copy-ledger contracts remain unchanged.
```
...
- Physical closure, 2026-08-03: the current iOS partnership has a nonzero Inbox checkpoint/MODSEQ, `minimum_uid=1`, 6,243 saved identities, zero pending commands, and `MoreAvailable=false`. Earlier server evidence proved exact saved-UID reconciliation and sustained no-change polling. The owner confirmed iOS Exchange, iOS IMAP, and macOS Mail all look correct. Historical already-junked messages and distinct current Inbox copies are separate identities; no subject-only mutation was performed.
...
- Physical result: iOS advanced beyond the original 25-message ceiling, exhausted the complete collection, and returned stable no-change responses. The 2026-08-03 checkpoint assertion found 6,243 saved Inbox identities, a nonzero key/MODSEQ, `minimum_uid=1`, zero pending commands, and `MoreAvailable=false`.
- Folder-state follow-up: the earlier checkpoint reconciled exact saved UIDs. The IMAP account consistently shows the historical spam examples in Junk, while same-sender/subject Inbox messages have distinct identities. No subject-only move was performed, and future investigation must continue to use exact UID/Message-ID identity.

## 2026-07-31 iOS Exchange MIME Body Hotfix

- Reproduction: iOS first synchronized Inbox previews with `BodyPreference Type=1` and `TruncationSize=500`, then opened a message with a Sync `Fetch`, `MIMESupport=2`, and `BodyPreference Type=4` but no `TruncationSize`. The old normalization inherited 500 bytes, often ending before the MIME header/body boundary.
- Fix: a present body preference with an omitted truncation size now selects the existing bounded complete-content ceiling of 10 MiB. Explicit truncation remains bounded as requested, and a request with no new body preference still reuses its persisted options.
- Regression proof: the new unit starts with the exact 500-byte preview state, sends the iOS MIME Fetch shape, and asserts the returned MIME contains the synthetic body and is not truncated. The authenticated ActiveSync smoke now performs the same Fetch before read/unread mutations.
- Automated proof: backend 254 total / 251 pass / three documented optional skips; focused ActiveSync 27/27; frontend 84/84 through the repository integration gate; TypeScript build, Bash syntax, full integration, and `git diff --check` pass. The authenticated smoke remains credential-gated in this session.
- Live proof: repository and deployed hashes match for the five affected `eas-mail-sync` artifacts; direct/public ActiveSync `OPTIONS` return `200`; Nginx is valid; `openmailstack.service` is active/running with `NRestarts=0`; the warning journal is empty; and the complete staging smoke passes. Rollback: `/var/backups/openmailstack/20260731T203319Z-eas-mime-body/`.
- Physical closure, 2026-08-03: the owner confirmed iOS Exchange works normally alongside iOS IMAP and identified Sieve as the only remaining mail issue. This closes the earlier empty-body symptom; the mandatory canary continues to exercise the exact full-MIME Fetch path on every guarded release.
