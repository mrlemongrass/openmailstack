...
INHERITED_GUARDED_CALENDAR_TOMBSTONE_REPAIR_APPROVAL="${OMS_GUARDED_CALENDAR_TOMBSTONE_REPAIR_APPROVAL:-}"
readonly INHERITED_GUARDED_CALENDAR_TOMBSTONE_REPAIR_APPROVAL
...
if [[ "${OMS_PROTOCOL_GUARDED_DEPLOY:-0}" == "1" ]]; then
    CALENDAR_TOMBSTONE_REPAIR_APPROVAL="${INHERITED_GUARDED_CALENDAR_TOMBSTONE_REPAIR_APPROVAL}"
else
    # One-time destructive authorization is never inherited from a previous
    # runtime or persistent config. A future install must supply a freshly
    # reviewed manifest in this process environment again.
    CALENDAR_TOMBSTONE_REPAIR_APPROVAL="${INHERITED_CALENDAR_TOMBSTONE_REPAIR_APPROVAL}"
fi
if (( ${#CALENDAR_TOMBSTONE_REPAIR_APPROVAL} > 8192 )) \
    || [[ "${CALENDAR_TOMBSTONE_REPAIR_APPROVAL}" == *$'\n'* \
        || "${CALENDAR_TOMBSTONE_REPAIR_APPROVAL}" == *$'\r'* ]] \
    || [[ -n "${CALENDAR_TOMBSTONE_REPAIR_APPROVAL}" \
        && ! "${CALENDAR_TOMBSTONE_REPAIR_APPROVAL}" =~ ^[A-Za-z0-9_-]+$ ]]; then
    echo -e "${RED}Error: OMS_CALENDAR_TOMBSTONE_REPAIR_APPROVAL is malformed.${NC}" >&2
    exit 1
fi
...
render_backend_env() {
    ...
    (
        umask 077
        {
            ...
            if [[ -n "${CALENDAR_TOMBSTONE_REPAIR_APPROVAL}" ]]; then
                write_env_line "OMS_CALENDAR_TOMBSTONE_REPAIR_APPROVAL" "${CALENDAR_TOMBSTONE_REPAIR_APPROVAL}"
            fi
