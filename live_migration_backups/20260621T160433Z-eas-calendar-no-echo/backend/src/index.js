...
async function saveActiveSyncCalendarEvent(calendarId, uid, ical) {
    await db_1.pool.query(`INSERT INTO events (calendar_id, uid, ical_data) VALUES (?, ?, ?)
         ON DUPLICATE KEY UPDATE ical_data = ?`, [calendarId, uid, ical, ical]);
    ...
    if (cmd === 'Sync') {
        ...
        if (isContactsCollection(collectionId)) {
            ...
            try {
                ...
                const addNodes = shouldSendContacts
                    ? (await (0, contact_utils_1.listContacts)(creds.user)).map(contact => ({
                        tag: 'Add',
                        page: 0,
                        children: [
        ...
        if (collectionId === 'mock-calendar' || collectionId.startsWith('cal-')) {
            ...
            try {
                ...
                if (collectionId.startsWith('cal-')) {
                    const calendarId = collectionId.slice(4);
                    const [rows] = await db_1.pool.query('SELECT * FROM calendars WHERE id = ? AND user_id = ? LIMIT 1', [calendarId, creds.user]);
                    if (rows.length === 0) {
                        const notFoundAst = {
                            tag: "Sync",
                            page: 0,
                ...
                for (const commandNode of commandsNode?.children || []) {
                    ...
                    if (commandNode.tag === 'Add') {
                        ...
                        await saveActiveSyncCalendarEvent(calendar.id, uid, ical);
                        ...
                        if (serverId && applicationData) {
                            ...
                            const [existingRows] = await db_1.pool.query('SELECT ical_data FROM events WHERE calendar_id = ? AND uid = ? LIMIT 1', [calendar.id, uid]);
                ...
                if (calendarChanged) {
                    const [updatedCalendars] = await db_1.pool.query('SELECT * FROM calendars WHERE id = ? AND user_id = ? LIMIT 1', [calendar.id, creds.user]);
...
app.listen(config_1.serverConfig.port, config_1.serverConfig.host, () => {
