...
requireText('webmail-backend/migrations/017_scheduler_availability_exclusions.sql', /scheduler_availability_exclusions[\s\S]*holiday[\s\S]*out_of_office/, 'Holiday and out-of-office migration is missing');
requireText('webmail-backend/migrations/018_scheduler_waitlist.sql', /scheduler_waitlist_entries[\s\S]*promoted_booking_id/, 'Waitlist migration is missing');
requireText('webmail-backend/migrations/019_scheduler_recurring_bookings.sql', /series_id[\s\S]*series_index[\s\S]*series_count/, 'Recurring-booking migration is missing');
requireText('webmail-backend/migrations/020_scheduler_meeting_polls.sql', /scheduler_meeting_polls[\s\S]*scheduler_poll_options[\s\S]*scheduler_poll_votes/, 'Meeting-poll migration is missing');
requireText('webmail-backend/migrations/021_scheduler_lifecycle_delegation.sql', /completed[\s\S]*no_show[\s\S]*booked_by_username/, 'Lifecycle and delegation migration is missing');
requireText('webmail-backend/migrations/022_scheduler_public_distribution.sql', /public_accent_color[\s\S]*locale[\s\S]*locked_time_zone[\s\S]*attribution/, 'Public distribution migration is missing');
requireText('webmail-backend/migrations/023_scheduler_portability.sql', /scheduler_import_runs[\s\S]*calendly[\s\S]*calcom/, 'Scheduler portability migration is missing');
...
requireText('webmail-backend/src/scheduler/store.ts', /promoteWaitlist[\s\S]*status='promoted'/, 'Waitlist capacity release must promote a pending party');
requireText('webmail-backend/src/scheduler/store.ts', /GET_LOCK\(\?, 10\)[\s\S]*Recurring occurrence/, 'Recurring booking creation must serialize idempotent series and preserve local time');
requireText('webmail-backend/src/scheduler/phase2-store.ts', /createPoll[\s\S]*votePoll[\s\S]*finalizePoll/, 'Meeting poll lifecycle is missing');
requireText('webmail-backend/src/scheduler/phase2-store.ts', /spreadsheetSafe[\s\S]*exportBookingsCsv/, 'Booking CSV export must neutralize spreadsheet formulas');
requireText('webmail-backend/src/scheduler/router.ts', /bookings\/on-behalf[\s\S]*bookings\/:id\/outcome/, 'Owner delegation and outcome routes are missing');
requireText('webmail-backend/src/scheduler/router.ts', /scheduler\/v1\/export[\s\S]*scheduler\/v1\/import/, 'Scheduler export and migration routes are missing');
requireText('webmail-frontend/src/scheduler/AvailabilityPanel.tsx', /Holidays and out of office/, 'Owner availability exclusions are missing');
requireText('webmail-frontend/src/scheduler/SchedulerToolsPanel.tsx', /Book on behalf[\s\S]*Meeting polls[\s\S]*Capacity waitlist[\s\S]*Share and embed[\s\S]*Export and migration/, 'Phase 2 owner tools are missing');
requireText('webmail-frontend/src/scheduler/PublicScheduler.tsx', /recurrenceCount[\s\S]*attribution[\s\S]*joinPublicWaitlist/, 'Public recurrence, attribution, and waitlist flows are missing');
requireText('webmail-frontend/src/scheduler/PublicScheduler.tsx', /query\.get\('embed'\) !== 'inline'/, 'Inline embed chrome control is missing');
requireText('webmail-frontend/src/scheduler/PublicScheduler.tsx', /oms\.scheduler\.booking\.confirmed/, 'Embed confirmation event contract is missing');
requireText('webmail-frontend/src/scheduler/PublicScheduler.tsx', /query\.get\('name'\)[\s\S]*query\.get\('email'\)/, 'Public booking prefill allowlist is missing');
