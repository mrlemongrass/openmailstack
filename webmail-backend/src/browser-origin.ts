...
function normalizedOrigin(value: string): NormalizedOrigin | null {
    if (!value || value !== value.trim() || value === 'null' || !ORIGIN_ONLY.test(value)) return null;
    try {
...
function requestTargetOrigin(req: OriginRequest): NormalizedOrigin | null {
    const host = req.headers.host;
    const forwardedProto = req.headers['x-forwarded-proto'];
    if (typeof host !== 'string' || !host || Array.isArray(forwardedProto)) return null;
...
export function browserRequestHasSameOrigin(req: OriginRequest): boolean {
...
export function requireSameOriginBrowserRequest(req: Request, res: Response, next: NextFunction): void {
...
export function allowSameOriginSocketRequest(
    req: IncomingMessage,
