...
function parseEventComponent(uid, componentBody, timeZones, fallback, isTask = false) {
    ...
    const activeSyncAttendees = attendeeLines.map(line => {
        ...
        const status = { 'NEEDS-ACTION': '0', TENTATIVE: '2', ACCEPTED: '3', DECLINED: '4' }[partstat || ''];
        const type = { 'REQ-PARTICIPANT': '1', 'OPT-PARTICIPANT': '2', 'NON-PARTICIPANT': '3' }[role || ''];
    ...
    const categories = categoryLines
        .flatMap(line => (0, structured_text_1.splitEscapedTextList)(line.slice(line.indexOf(':') + 1)).map(unescapeIcalText))
        .filter(Boolean);
    ...
    return {
        ...
        sensitivity: /^[0-3]$/.test(activeSyncSensitivity || '')
            ...
            : fallback?.sensitivity || { PUBLIC: '0', PERSONAL: '1', PRIVATE: '2', CONFIDENTIAL: '3' }[classValue || ''] || '0',
