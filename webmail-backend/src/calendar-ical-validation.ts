...
export const MAX_ICAL_DOCUMENT_BYTES = 8 * 1024 * 1024;
export const MAX_ICAL_RESOURCE_BYTES = 1024 * 1024;
export const MAX_ICAL_AGGREGATE_RESOURCE_BYTES = 16 * 1024 * 1024;
export const MAX_ICAL_RESOURCE_COMPONENTS = 10_000;
export const MAX_ICAL_UID_CHARACTERS = 255;
export const MAX_ICAL_UID_BYTES = MAX_ICAL_UID_CHARACTERS * 4;
...
const ALLOWED_NESTED_COMPONENTS: Record<string, ReadonlySet<string>> = {
...
export class ICalendarValidationError extends Error {
...
interface ParsedTopLevelComponent {
    type: string;
    lines: string[];
...
function validateCalendarTextCharacters(source: string): void {
...
function unfoldIcalendarLines(source: string): string[] {
    const physicalLines = source.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
    const logicalLines: string[] = [];
    for (const line of physicalLines) {
        if (/^[ \t]/.test(line)) {
            if (logicalLines.length === 0) validationError('Invalid iCalendar line folding');
...
function componentBoundary(line: string): { kind: 'BEGIN' | 'END'; type: string } | null {
    const match = line.match(/^(BEGIN|END):([A-Z0-9-]+)$/i);
    if (!match) return null;
    return { kind: match[1].toUpperCase() as 'BEGIN' | 'END', type: match[2].toUpperCase() };
}

function propertyName(line: string): string {
    const separator = line.indexOf(':');
    if (separator < 1) validationError('Invalid iCalendar property');
...
function directPropertyValues(component: ParsedTopLevelComponent, name: string): string[] {
...
function validateCalendarProperties(
    ...
    mode: NonNullable<ICalendarValidationOptions['mode']>,
): void {
    const values = (name: string) => calendarProperties
        .filter(line => line && propertyName(line) === name)
        .map(propertyValue);
...
function validateRequiredResourceProperties(component: ParsedTopLevelComponent): void {
    if (!RESOURCE_COMPONENT_TYPES.has(component.type)) return;
...
function directUid(component: ParsedTopLevelComponent, maxUidBytes: number): string | null {
    const values = component.directProperties
        .filter(line => line && propertyName(line) === 'UID')
        .map(line => line.slice(line.indexOf(':') + 1));
    if (!RESOURCE_COMPONENT_TYPES.has(component.type)) return values[0] ?? null;
    if (values.length !== 1) validationError(`${component.type} must contain exactly one direct UID property`);
    const uid = values[0];
    if (!uid || uid.endsWith(' ') || /[\x00-\x1f\x7f]/.test(uid)) {
        validationError(`${component.type} UID is invalid`);
    }
    if (Array.from(uid).length > MAX_ICAL_UID_CHARACTERS
...
function directRecurrenceId(component: ParsedTopLevelComponent, maxUidBytes: number): string | null {
    if (!RESOURCE_COMPONENT_TYPES.has(component.type)) return null;
    const values = component.directProperties
        .filter(line => line && propertyName(line) === 'RECURRENCE-ID')
        .map(line => line.slice(line.indexOf(':') + 1));
    if (values.length > 1) validationError(`${component.type} contains duplicate RECURRENCE-ID properties`);
...
export function validateICalendarDocument(
    input: string | Buffer,
    options: ICalendarValidationOptions = {},
): ValidatedICalendarDocument {
    const maxDocumentBytes = options.maxDocumentBytes ?? MAX_ICAL_DOCUMENT_BYTES;
    const maxResourceBytes = options.maxResourceBytes ?? MAX_ICAL_RESOURCE_BYTES;
    const maxAggregateResourceBytes = options.maxAggregateResourceBytes ?? MAX_ICAL_AGGREGATE_RESOURCE_BYTES;
    const maxResourceComponents = options.maxResourceComponents ?? MAX_ICAL_RESOURCE_COMPONENTS;
    const maxUidBytes = options.maxUidBytes ?? MAX_ICAL_UID_BYTES;
    const mode = options.mode ?? 'stored-resource';
    const sourceBuffer = Buffer.isBuffer(input) ? input : Buffer.from(input, 'utf8');
    if (sourceBuffer.length > maxDocumentBytes) validationError('iCalendar document is too large');
    let source: string;
    try {
    ...
    const components: ParsedTopLevelComponent[] = [];
    let openTopLevel: ParsedTopLevelComponent | null = null;
    ...
    for (const line of lines) {
        ...
        if (boundary?.kind === 'BEGIN') {
            ...
            if (calendarClosed || boundary.type === 'VCALENDAR') validationError('Invalid nested iCalendar root');
            if (stack.length === 1) {
        ...
        if (boundary?.kind === 'END') {
            ...
            if (stack.length >= 2) openTopLevel?.lines.push(line);
            stack.pop();
            if (stack.length === 1 && openTopLevel) {
                components.push(openTopLevel);
                openTopLevel = null;
            } else if (stack.length === 0) {
                if (boundary.type !== 'VCALENDAR') validationError('Invalid iCalendar root closure');
                calendarClosed = true;
            }
            continue;
        }
        ...
        if (stack.length === 0) {
            if (line.trim()) validationError('Data outside VCALENDAR is not allowed');
            continue;
        }
        if (!line) continue;
        const name = propertyName(line);
        if (name === 'BEGIN' || name === 'END') validationError('Malformed iCalendar component boundary');
        if (stack.length === 1) {
            calendarProperties.push(line);
        } else {
            openTopLevel?.lines.push(line);
            if (stack.length === 2) openTopLevel?.directProperties.push(line);
        }
    }
    ...
    if (resourceComponents.length > maxResourceComponents) validationError('iCalendar contains too many resources');
    ...
    const unfoldedUids = resourceComponents.map(component => component.uid as string);
