...
import { MAX_ICAL_DOCUMENT_BYTES } from './calendar-ical-validation';

export const MAX_CALENDAR_SUBSCRIPTION_REDIRECTS = 3;
export const MAX_CALENDAR_SUBSCRIPTION_FETCH_MS = 15_000;
export const MAX_CALENDAR_SUBSCRIPTION_BODY_BYTES = MAX_ICAL_DOCUMENT_BYTES;
export const MAX_CALENDAR_SUBSCRIPTION_URL_BYTES = 4096;

export class CalendarSubscriptionHttpError extends Error {
...
function normalizeRedirectLimit(value: unknown): number {
    if (value === undefined) return MAX_CALENDAR_SUBSCRIPTION_REDIRECTS;
    const numeric = Number(value);
    if (!Number.isFinite(numeric) || !Number.isInteger(numeric)) {
        httpError('Calendar subscription redirect limit is invalid');
    }
    return Math.max(0, Math.min(numeric, MAX_CALENDAR_SUBSCRIPTION_REDIRECTS));
}

const defaultDependencies: CalendarSubscriptionHttpDependencies = {
    lookup: async hostname => dnsLookup(hostname, { all: true, verbatim: true }),
    request: httpsRequest,
};

function httpError(message: string): never {
...
export function validateCalendarSubscriptionUrl(value: unknown): URL {
    if (typeof value !== 'string' || !value.trim()
        || Buffer.byteLength(value.trim(), 'utf8') > MAX_CALENDAR_SUBSCRIPTION_URL_BYTES) {
    ...
    if (parsed.protocol !== 'https:') httpError('Calendar subscription URL must use HTTPS');
    if (parsed.username || parsed.password) httpError('Calendar subscription URL cannot contain credentials');
    if (!parsed.hostname) httpError('Calendar subscription URL is invalid');
    return parsed;
}

export function calendarSubscriptionLogLabel(value: unknown): string {
    try {
...
function ipv6Prefix(value: bigint, prefix: string, length: number): boolean {
...
function ipv6IsPublic(address: string): boolean {
    ...
    if (ipv6Prefix(value, '2001:0000::', 32)) return false; // Teredo
    if (ipv6Prefix(value, '2001:0002::', 48)) return false; // benchmarking
    if (ipv6Prefix(value, '2001:0010::', 28)) return false; // ORCHIDv1
    if (ipv6Prefix(value, '2001:0020::', 28)) return false; // ORCHIDv2
    if (ipv6Prefix(value, '2001:0db8::', 32)) return false; // documentation
    if (ipv6Prefix(value, '2002::', 16)) return false; // 6to4 can embed private IPv4
    if (ipv6Prefix(value, '3fff::', 20)) return false; // documentation
    return true;
}

export function isPublicCalendarSubscriptionAddress(address: string): boolean {
    const normalized = hostnameWithoutBrackets(address);
    if (isIP(normalized) === 4) return ipv4IsPublic(normalized);
    if (isIP(normalized) === 6) return ipv6IsPublic(normalized);
    return false;
}

function deadlinePromise<T>(promise: Promise<T>, deadline: number, message: string): Promise<T> {
    const remaining = deadline - Date.now();
    if (remaining <= 0) return Promise.reject(new CalendarSubscriptionHttpError(message));
    return new Promise<T>((resolve, reject) => {
        const timer = setTimeout(() => reject(new CalendarSubscriptionHttpError(message)), remaining);
        promise.then(
            value => { clearTimeout(timer); resolve(value); },
            () => { clearTimeout(timer); reject(new CalendarSubscriptionHttpError(message)); },
        );
    });
}
...
async function resolvePinnedAddress(
    url: URL,
    dependencies: CalendarSubscriptionHttpDependencies,
    deadline: number,
): Promise<CalendarSubscriptionLookupAddress> {
    ...
    const addresses = literalFamily
        ...
        : await deadlinePromise(
            ...
            deadline,
            'Calendar subscription DNS lookup timed out',
        );
...
function requestPinned(
    url: URL,
    pinned: CalendarSubscriptionLookupAddress,
    dependencies: CalendarSubscriptionHttpDependencies,
    deadline: number,
    maxBodyBytes: number,
    ...
    return new Promise((resolve, reject) => {
        ...
        const finish = (error: CalendarSubscriptionHttpError | null, value?: SingleResponse) => {
            if (settled) return;
            settled = true;
            clearTimeout(timer);
        ...
        const remaining = deadline - Date.now();
        if (remaining <= 0) return reject(new CalendarSubscriptionHttpError('Calendar subscription request timed out'));
        ...
        try {
            request = dependencies.request({
                ...
                agent: false,
                servername: isIP(hostnameWithoutBrackets(url.hostname)) ? undefined : hostnameWithoutBrackets(url.hostname),
                headers: {
                ...
                if ([301, 302, 303, 307, 308].includes(status)) {
                    ...
                    response.destroy();
                    if (!location) return finish(new CalendarSubscriptionHttpError('Calendar subscription redirect is invalid'));
            ...
            request.once('error', error => finish(
                error instanceof CalendarSubscriptionHttpError
                    ? error
...
export async function fetchCalendarSubscription(
    value: unknown,
    options: CalendarSubscriptionFetchOptions = {},
): Promise<Buffer> {
    const dependencies = { ...defaultDependencies, ...(options.dependencies || {}) };
    const timeoutMs = Math.max(1, Math.min(
        Number(options.timeoutMs) || MAX_CALENDAR_SUBSCRIPTION_FETCH_MS,
        MAX_CALENDAR_SUBSCRIPTION_FETCH_MS,
    ));
    const maxBodyBytes = Math.max(1, Math.min(
        Number(options.maxBodyBytes) || MAX_CALENDAR_SUBSCRIPTION_BODY_BYTES,
        MAX_CALENDAR_SUBSCRIPTION_BODY_BYTES,
    ));
    ...
    try {
        for (let redirectCount = 0; ; redirectCount += 1) {
            ...
            try {
                ...
                if (error instanceof CalendarSubscriptionHttpError) throw error;
