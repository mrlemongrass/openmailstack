...
const ensureSubscriptionSchema = async () => {
    if (!schemaPromise) {
        schemaPromise = (async () => {
            ...
            const names = new Set((cols as any[]).map((c: any) => c.COLUMN_NAME));
...
const runSubscriptionFetcher = async () => {
    try {
        ...
        const [calendars]: any = await pool.query(
            `SELECT id, user_id, subscribed_url, last_fetched_at, last_fetch_error
             FROM calendars
        ...
        for (const cal of calendars) {
            try {
                ...
                for (const vevent of matches) {
                    ...
                    await pool.query(
                        `INSERT INTO events (calendar_id, uid, ical_data)
                         VALUES (?, ?, ?)
                ...
                await pool.query(
                    'UPDATE calendars SET last_fetched_at = NOW(), last_fetch_error = NULL WHERE id = ?',
                    [cal.id]
                );
                await pool.query('UPDATE calendars SET sync_token = sync_token + 1 WHERE id = ?', [cal.id]);
                ...
                await pool.query(
                    'UPDATE calendars SET last_fetched_at = NOW(), last_fetch_error = ? WHERE id = ?',
                    [err.message.substring(0, 500), cal.id]
                );
            }
        }
    } catch (err: any) {
