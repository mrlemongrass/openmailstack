...
exports.normalizeCalendarSharePermission = exports.canWriteActiveSyncCalendar = exports.ActiveSyncCalendarFieldError = exports.MAX_ACTIVE_SYNC_CALENDAR_EXCEPTIONS = exports.MAX_ACTIVE_SYNC_CALENDAR_BODY_BYTES = void 0;
exports.resolveActiveSyncCalendarAccessRole = resolveActiveSyncCalendarAccessRole;
...
exports.storedIcalEventToActiveSyncApplicationData = storedIcalEventToActiveSyncApplicationData;
const calendar_format_1 = require("./calendar-format");
const eas_timezone_1 = require("./eas-timezone");
exports.MAX_ACTIVE_SYNC_CALENDAR_BODY_BYTES = 1024 * 1024;
exports.MAX_ACTIVE_SYNC_CALENDAR_EXCEPTIONS = 256;
const MAX_ACTIVE_SYNC_CALENDAR_CATEGORY_BYTES = 4096;
const MAX_ACTIVE_SYNC_CALENDAR_EXCEPTION_TEXT_BYTES = 4096;
class ActiveSyncCalendarFieldError extends Error {
    constructor() {
        super('ActiveSync calendar field is unsupported or too large');
        this.name = 'ActiveSyncCalendarFieldError';
    }
}
exports.ActiveSyncCalendarFieldError = ActiveSyncCalendarFieldError;
const canWriteActiveSyncCalendar = (accessRole) => accessRole === 'owner' || accessRole === 'write';
exports.canWriteActiveSyncCalendar = canWriteActiveSyncCalendar;
const normalizeCalendarSharePermission = (value) => value === 'read' || value === 'write' ? value : null;
exports.normalizeCalendarSharePermission = normalizeCalendarSharePermission;
function resolveActiveSyncCalendarAccessRole(row, user) {
    if (!row)
        return null;
    const baseRole = String(row.user_id || '') === user
        ? 'owner'
        : (0, exports.normalizeCalendarSharePermission)(row.permission);
    if (!baseRole)
        return null;
...
function truncateCalendarBody(value) {
    const safeValue = value.replace(/\0/g, '\uFFFD');
    const source = Buffer.from(safeValue, 'utf8');
    if (source.length <= exports.MAX_ACTIVE_SYNC_CALENDAR_BODY_BYTES) {
        return { data: safeValue, bytes: source.length, truncated: false };
    }
    let end = exports.MAX_ACTIVE_SYNC_CALENDAR_BODY_BYTES;
...
function boundedCalendarText(value, maxBytes = 8192) {
...
function icalPropertyName(line) {
    return line.slice(0, Math.max(0, line.indexOf(':'))).split(';')[0].toUpperCase();
}
function existingEventBlocks(ical) {
    const blocks = [];
    let current = null;
    let depth = 0;
...
function directEventProperties(block) {
    const result = [];
    let nested = 0;
...
function assertSupportedCalendarApplicationData(applicationData) {
    const topSeen = new Set();
    const scalar = (node, maxBytes = 8192, allowEmpty = true) => node && (!node.children || node.children.length === 0)
        && (node.content === undefined && allowEmpty
            || typeof node.content === 'string'
                && Buffer.byteLength(node.content, 'utf8') <= maxBytes
                && !/[\u0000-\u001f\u007f]/.test(node.content)
                && (allowEmpty || node.content.length > 0));
    const multilineScalar = (node, maxBytes) => node && (!node.children || node.children.length === 0) && typeof node.content === 'string'
        && Buffer.byteLength(node.content, 'utf8') <= maxBytes
        && !/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(node.content);
    const validDate = (value) => Boolean(parseActiveSyncCalendarDate(value));
    const validBody = (node) => {
        ...
        for (const child of node.children) {
            if (child?.page !== 17 || seen.has(child.tag)
                ...
                || !(child.tag === 'Data'
                    ? multilineScalar(child, exports.MAX_ACTIVE_SYNC_CALENDAR_BODY_BYTES)
                    : scalar(child, 16)))
                return false;
            seen.add(child.tag);
        }
        const bodyValue = (tag) => nodeText(node.children.find((child) => child.tag === tag));
    ...
    const validCategories = (node) => Array.isArray(node?.children)
        && node.children.length <= 128
        && node.children.every((child) => child?.page === 4 && child.tag === 'Category'
            && scalar(child, MAX_ACTIVE_SYNC_CALENDAR_CATEGORY_BYTES));
    const validAttendees = (node) => Array.isArray(node?.children)
        && node.children.length <= 128
        && node.children.every((attendee) => {
            const seen = new Set();
            return attendee?.page === 4 && attendee.tag === 'Attendee'
                && Array.isArray(attendee.children)
                && attendee.children.every((child) => child?.page === 4 && !seen.has(child.tag)
    ...
    for (const field of applicationData?.children || []) {
        ...
        if (field.tag === 'Recurrence') {
            ...
            const value = (tag) => nodeText(field.children.find((child) => child.tag === tag));
            const number = (tag) => {
                const raw = value(tag);
        ...
        if (field.tag === 'Categories') {
            if (!validCategories(field))
                throw new ActiveSyncCalendarFieldError();
            continue;
        }
        if (field.tag === 'Attendees') {
            if (!validAttendees(field))
                throw new ActiveSyncCalendarFieldError();
            continue;
        }
        if (field.tag === 'Exceptions') {
            if (!Array.isArray(field.children) || field.children.length > exports.MAX_ACTIVE_SYNC_CALENDAR_EXCEPTIONS) {
                throw new ActiveSyncCalendarFieldError();
            }
            ...
            for (const exception of field.children) {
                ...
                for (const child of exception.children) {
                    if (!child || seen.has(`${child.page}:${child.tag}`))
                        throw new ActiveSyncCalendarFieldError();
                    ...
                    if (child.tag === 'Categories') {
                        if (!validCategories(child))
                            throw new ActiveSyncCalendarFieldError();
                        continue;
                    }
                    if (child.tag === 'Attendees') {
                        if (!validAttendees(child))
                            throw new ActiveSyncCalendarFieldError();
                        continue;
                    }
...
function activeSyncAttendeeValues(node) {
    return (node?.children || []).map((attendee) => ({
...
function appendIcalAttendees(lines, attendees) {
    const statusMap = { '0': 'NEEDS-ACTION', '2': 'TENTATIVE', '3': 'ACCEPTED', '4': 'DECLINED' };
    const roleMap = { '1': 'REQ-PARTICIPANT', '2': 'OPT-PARTICIPANT', '3': 'NON-PARTICIPANT' };
...
function calendarApplicationDataWithOmittedClears(applicationData, omittedFieldsToClear) {
    ...
    for (const identity of omittedFieldsToClear) {
        if (present.has(identity) || !identity.startsWith('4:'))
            continue;
...
function activeSyncCalendarApplicationDataToIcal(uid, applicationData, existingIcal = '', omittedFieldsToClear = new Set()) {
    ...
    if (recurrenceNode && recurrenceNode.children?.length === 0) {
        ...
        if (Number.isInteger(monthOfYear) && monthOfYear >= 1 && monthOfYear <= 12)
            rrule += `;BYMONTH=${monthOfYear}`;
        const firstDay = Number(childText(recurrenceNode, 'FirstDayOfWeek'));
        if (childText(recurrenceNode, 'FirstDayOfWeek'))
            rrule += `;WKST=${dayNames[firstDay]}`;
        rruleLine = rrule;
    }
        ...
        if (deletedOccurrenceIds.length > 0) {
            if (isAllDay) {
                ...
                lines.push(`EXDATE;TZID=${timeZone}:${deletedOccurrenceIds
                    .map(date => (0, eas_timezone_1.formatIcalWallTime)(date, timeZone)).join(',')}`);
            }
            else {
    ...
    if (attendeesNode)
        appendIcalAttendees(lines, masterAttendees);
    ...
    if (categoriesNode) {
        const categories = (categoriesNode.children || []).map((category) => nodeText(category)).filter(Boolean);
        if (categories.length)
            lines.push(`CATEGORIES:${categories.map(icalEscape).join(',')}`);
    }
    const sensitivityNode = childNode(applicationData, 'Sensitivity');
    if (sensitivityNode) {
        const raw = nodeText(sensitivityNode);
        const value = { '0': 'PUBLIC', '1': 'PERSONAL', '2': 'PRIVATE', '3': 'CONFIDENTIAL' }[raw];
        if (value)
            lines.push(`CLASS:${value}`, `X-OMS-ACTIVESYNC-SENSITIVITY:${raw}`);
    }
    ...
    if (conferenceNode && nodeText(conferenceNode))
        lines.push(`CONFERENCE;VALUE=URI:${icalEscape(nodeText(conferenceNode))}`);
    if (externalConferenceNode && nodeText(externalConferenceNode))
        lines.push(`X-OMS-ONLINE-MEETING-EXTERNAL:${icalEscape(nodeText(externalConferenceNode))}`);
    ...
    if (attendeesNode)
        managedProperties.add('ATTENDEE');
    if (organizerEmailNode || organizerNameNode)
        managedProperties.add('ORGANIZER');
    if (categoriesNode)
        managedProperties.add('CATEGORIES');
    if (sensitivityNode)
        managedProperties.add('CLASS');
    if (sensitivityNode)
        managedProperties.add('X-OMS-ACTIVESYNC-SENSITIVITY');
    if (meetingStatusNode) {
        managedProperties.add('STATUS');
        managedProperties.add('X-OMS-ACTIVESYNC-MEETING-STATUS');
    }
    if (conferenceNode)
        managedProperties.add('CONFERENCE');
    if (externalConferenceNode)
        managedProperties.add('X-OMS-ONLINE-MEETING-EXTERNAL');
    if (responseRequestedNode)
        managedProperties.add('X-OMS-RESPONSE-REQUESTED');
    if (disallowNewTimeProposalNode)
        managedProperties.add('X-OMS-DISALLOW-NEW-TIME-PROPOSAL');
    if (incomingBusyStatusNode)
        managedProperties.add('X-OMS-ACTIVESYNC-BUSY-STATUS');
    if (mainBlock) {
        for (const property of directEventProperties(mainBlock)) {
            if (!managedProperties.has(icalPropertyName(property)))
                lines.push(property);
        }
    }
    ...
    if (incomingExceptions) {
        for (const exception of incomingExceptions.children || []) {
            ...
            const masterCategories = categoriesNode
                ? (categoriesNode.children || []).map((category) => nodeText(category)).filter(Boolean)
                : existing?.categories || [];
            const exceptionCategories = exceptionCategoriesNode
                ? (exceptionCategoriesNode.children || []).map((category) => nodeText(category)).filter(Boolean)
            ...
            lines.push('BEGIN:VEVENT', `UID:${icalEscape(eventUid)}`, `DTSTAMP:${(0, calendar_format_1.formatActiveSyncDate)(exceptionDtStamp)}`);
            if (isAllDay) {
            ...
            if (exceptionLocationNode || exceptionLocation)
                lines.push(`LOCATION:${icalEscape(exceptionLocation)}`);
            if (exceptionBodyNode || exceptionDescription)
                lines.push(`DESCRIPTION:${icalEscape(exceptionDescription)}`);
            ...
            lines.push(`X-OMS-ACTIVESYNC-BUSY-STATUS:${exceptionBusyStatus}`, `TRANSP:${exceptionBusyStatus === '0' ? 'TRANSPARENT' : 'OPAQUE'}`, `STATUS:${['5', '7', '13', '15'].includes(exceptionMeetingStatus) ? 'CANCELLED' : 'CONFIRMED'}`, `X-OMS-ACTIVESYNC-MEETING-STATUS:${exceptionMeetingStatus}`);
            if (JSON.stringify(exceptionAttendees) !== JSON.stringify(masterAttendees)) {
                if (exceptionAttendees.length === 0)
                    lines.push('X-OMS-ACTIVESYNC-ATTENDEES-CLEARED:1');
                else
                    appendIcalAttendees(lines, exceptionAttendees);
            }
...
function recurrenceParts(raw) {
    ...
    for (const item of raw.split(';')) {
        ...
        if (!value || parts.has(key))
            throw new ActiveSyncCalendarFieldError();
        parts.set(key, value);
    }
    const supported = new Set(['FREQ', 'INTERVAL', 'COUNT', 'UNTIL', 'BYDAY', 'BYMONTHDAY', 'BYMONTH', 'BYSETPOS', 'WKST']);
    if ([...parts.keys()].some(key => !supported.has(key)))
        throw new ActiveSyncCalendarFieldError();
    return parts;
}
function recurrenceDaySelection(value) {
    if (!value)
        return null;
    const dayBits = { SU: 1, MO: 2, TU: 4, WE: 8, TH: 16, FR: 32, SA: 64 };
    let mask = 0;
    const positions = new Set();
    for (const token of value.split(',')) {
        const match = /^([+-]?\d+)?(SU|MO|TU|WE|TH|FR|SA)$/.exec(token);
        if (!match || mask & dayBits[match[2]])
            throw new ActiveSyncCalendarFieldError();
        mask |= dayBits[match[2]];
        if (match[1])
            positions.add(Number(match[1]));
    }
    if (!mask || positions.size > 1)
        throw new ActiveSyncCalendarFieldError();
    return { mask, position: positions.size === 1 ? [...positions][0] : null };
}
function recurrenceListNumber(parts, key, minimum, maximum) {
...
function activeSyncRecurrence(event) {
    ...
    if (bySetPositionValue !== undefined) {
        if (!/^(?:[1-4]|-1)$/.test(bySetPositionValue))
            throw new ActiveSyncCalendarFieldError();
    ...
    if (event.recurrence.frequency === 'DAILY') {
        ...
        type = '0';
        if (daySelection)
            recurrenceFields.push({ tag: 'DayOfWeek', page: 4, content: String(daySelection.mask) });
    }
        ...
        if (monthOfYear !== null || dayOfMonth !== null && daySelection)
            throw new ActiveSyncCalendarFieldError();
        if (daySelection) {
            if (bySetPosition === null)
                throw new ActiveSyncCalendarFieldError();
            type = '3';
            recurrenceFields.push({ tag: 'DayOfWeek', page: 4, content: String(daySelection.mask) }, { tag: 'WeekOfMonth', page: 4, content: String(bySetPosition === -1 ? 5 : bySetPosition) });
        }
        else {
            if (bySetPosition !== null)
                throw new ActiveSyncCalendarFieldError();
        ...
        if (dayOfMonth !== null && daySelection)
            throw new ActiveSyncCalendarFieldError();
        if (daySelection) {
            if (monthOfYear === null || bySetPosition === null)
                throw new ActiveSyncCalendarFieldError();
            type = '6';
            recurrenceFields.push({ tag: 'DayOfWeek', page: 4, content: String(daySelection.mask) }, { tag: 'WeekOfMonth', page: 4, content: String(bySetPosition === -1 ? 5 : bySetPosition) }, { tag: 'MonthOfYear', page: 4, content: String(monthOfYear) });
        }
        else {
            if (bySetPosition !== null)
                throw new ActiveSyncCalendarFieldError();
            if ((dayOfMonth === null) !== (monthOfYear === null))
                throw new ActiveSyncCalendarFieldError();
            type = '5';
            recurrenceFields.push({ tag: 'DayOfMonth', page: 4, content: String(dayOfMonth || wall.day) }, { tag: 'MonthOfYear', page: 4, content: String(monthOfYear || wall.month) });
        }
    }
    if (type === '6' && event.activeSyncRecurrenceDayOfWeekOmitted) {
        ...
        if (dayOfWeekIndex >= 0)
            recurrenceFields.splice(dayOfWeekIndex, 1);
    }
    if (['2', '3', '5', '6'].includes(type)) {
        const calendarType = event.activeSyncCalendarType ?? '1';
        if (!/^(?:[0-9]|1[0-2]|14|15|20)$/.test(calendarType))
            throw new ActiveSyncCalendarFieldError();
        ...
        if (['5', '6'].includes(type) && event.activeSyncIsLeapMonth !== undefined) {
            if (!/^[01]$/.test(event.activeSyncIsLeapMonth))
                throw new ActiveSyncCalendarFieldError();
...
function calendarEventToActiveSyncApplicationData(event) {
    ...
    if (event.location)
        applicationData.push({ tag: 'Location', page: 4, content: boundedCalendarText(event.location) });
    if (event.organizerEmail) {
        applicationData.push({ tag: 'OrganizerEmail', page: 4, content: boundedCalendarText(event.organizerEmail, 1024) });
        if (event.organizerName)
            applicationData.push({ tag: 'OrganizerName', page: 4, content: boundedCalendarText(event.organizerName) });
    }
    const attendees = event.activeSyncAttendees?.length
    ...
    if (event.recurrenceExceptionOverflow)
        throw new ActiveSyncCalendarFieldError();
    const exceptionNodesByOccurrence = new Map();
    ...
    for (const exception of event.recurrenceExceptions || []) {
        ...
        if (exceptionEvent.dtstamp.getTime() !== event.dtstamp.getTime()) {
            children.push({ tag: 'DtStamp', page: 4, content: (0, calendar_format_1.formatActiveSyncDate)(exceptionEvent.dtstamp) });
        }
        ...
        const exceptionAttendees = exceptionEvent.activeSyncAttendees !== undefined
    ...
    if (exceptionNodesByOccurrence.size > exports.MAX_ACTIVE_SYNC_CALENDAR_EXCEPTIONS) {
        throw new ActiveSyncCalendarFieldError();
    }
...
function storedIcalEventToActiveSyncApplicationData(storageUid, ical) {
    try {
        const event = (0, calendar_format_1.parseIcalEvent)(storageUid, ical);
        ...
        if (dates.some(date => {
            if (!(date instanceof Date) || !Number.isFinite(date.getTime()))
                return true;
            try {
                return !/^\d{8}T\d{6}Z$/.test((0, calendar_format_1.formatActiveSyncDate)(date));
            }
            catch {
                return true;
            }
        ...
        if (error instanceof ActiveSyncCalendarFieldError)
            throw error;
