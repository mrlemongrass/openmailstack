...
export function activeSyncContactApplicationDataToVCard(davUid: string, applicationData: ActiveSyncNode): string {
...
export function contactToActiveSyncApplicationData(contact: ActiveSyncContactRow, vcard = ''): ActiveSyncNode[] {
    ...
    emails.forEach((item, index) => {
        ...
        if (tag) data.push({ tag, page: 1, content: item.value });
    });

    const usedPhoneTags = new Set<string>();
