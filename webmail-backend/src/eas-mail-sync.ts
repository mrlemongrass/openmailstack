...
export class MailSyncStateError extends Error {
...
export function resolveActiveSyncWindowSize(
    syncKey: string,
    value: unknown,
    persistedWindowSize?: number,
): number {
    const fallback = syncKey === '0' ? DEFAULT_OPTIONS.windowSize : persistedWindowSize ?? DEFAULT_OPTIONS.windowSize;
    return normalizeActiveSyncWindowSize(value, fallback);
}
...
export function normalizeMailSyncOptions(
    ...
    const windowSize = normalizeActiveSyncWindowSize(values.windowSize, fallback.windowSize);
...
const boundedMailText = (value: unknown, maxBytes = 64 * 1024): string =>
    truncateUtf8Body(String(value || ''), maxBytes).data;
...
export async function activeSyncMailApplicationData(
    ...
    return [
        { tag: 'To', page: 2, content: boundedMailText((parsed.to as any)?.text) },
        { tag: 'From', page: 2, content: boundedMailText((parsed.from as any)?.text) },
        ...
        { tag: 'DisplayTo', page: 2, content: boundedMailText((parsed.to as any)?.text) },
...
export const MAX_MAIL_SYNC_REPLAY_BYTES = 8 * 1024 * 1024;
export const MAX_MAIL_SYNC_RESPONSE_BYTES = MAX_MAIL_SYNC_REPLAY_BYTES;
export const MAX_MAIL_SYNC_SOURCE_BYTES = 7 * 1024 * 1024;
export const MAX_MAIL_SYNC_KNOWN_ITEMS = 100_000;
export const MAX_MAIL_SYNC_KNOWN_ITEMS_BYTES = 4 * 1024 * 1024;
export const MAX_MAIL_SYNC_COMMANDS_BYTES = 256 * 1024;
export const MAX_MAIL_SYNC_ROW_BYTES = 13 * 1024 * 1024;
export const MAX_MAIL_SYNC_USER_BYTES = 64 * 1024 * 1024;
export const MAX_MAIL_SYNC_PARTNERSHIPS_PER_USER = 1_024;
export const MAIL_SYNC_STATE_TTL_MS = 180 * 24 * 60 * 60 * 1000;
const isMailSyncKey = (value: string): boolean => /^oms-mail-[0-9a-f]{48}$/.test(value);
const canonicalDecimal = (value: unknown, maxDigits = 20): boolean =>
    typeof value === 'string' && new RegExp(`^(?:0|[1-9][0-9]{0,${maxDigits - 1}})$`).test(value);

export function validateMailClientCommands(commands: any[], collectionId: string): { ok: true } | { ok: false } {
    if (!Array.isArray(commands) || commands.length > 512) return { ok: false };
    const seen = new Set<string>();
    const scalar = (node: any): boolean => node && (!node.children || node.children.length === 0)
    ...
    for (const command of commands) {
        ...
        if (!['Fetch', 'Change', 'Delete'].includes(command.tag)) return { ok: false };
        ...
        if (serverIdNode?.tag !== 'ServerId' || serverIdNode?.page !== 0 || !scalar(serverIdNode)
            ...
            || activeSyncMailMessageUid(collectionId, serverIdNode.content) === null) return { ok: false };
        const identity = `${command.tag}:${serverIdNode.content}`;
        if (seen.has(identity)) return { ok: false };
        seen.add(identity);
        if (command.tag === 'Change') {
            ...
            if (command.children.length !== 2 || appData?.tag !== 'ApplicationData' || appData?.page !== 0
                ...
                || appData.children.length !== 1) return { ok: false };
            const read = appData.children[0];
            if (read?.tag !== 'Read' || read?.page !== 2 || !scalar(read) || !/^[01]$/.test(read.content)) return { ok: false };
...
export function assertMailSyncRowBound(knownItems: string, commands: string, response: Buffer | null): void {
    const bytes = Buffer.byteLength(knownItems, 'utf8') + Buffer.byteLength(commands, 'utf8') + (response?.length || 0);
    if (bytes > MAX_MAIL_SYNC_ROW_BYTES) throw new MailSyncStateError('Mail sync state exceeds its aggregate row bound');
}
...
export const parseMailSyncKnownItems = (value: unknown): MailSyncKnownItems => {
    try {
        const parsed = JSON.parse(Buffer.isBuffer(value) ? value.toString('utf8') : String(value));
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('invalid');
        if (Object.keys(parsed).length > MAX_MAIL_SYNC_KNOWN_ITEMS
            || Buffer.byteLength(JSON.stringify(parsed), 'utf8') > MAX_MAIL_SYNC_KNOWN_ITEMS_BYTES) {
        ...
        for (const [uid, read] of Object.entries(parsed)) {
            if (!/^[1-9][0-9]{0,15}$/.test(uid) || !Number.isSafeInteger(Number(uid))
                || ![0, 1].includes(Number(read))) throw new Error('invalid');
...
const parseCommands = (value: unknown): MailSyncCommand[] => {
    try {
        ...
        if (Buffer.byteLength(serialized, 'utf8') > MAX_MAIL_SYNC_COMMANDS_BYTES) throw new Error('invalid');
        const parsed = JSON.parse(serialized);
        if (!Array.isArray(parsed) || parsed.length > 512 || parsed.some(command =>
            !command || !Number.isSafeInteger(command.uid) || command.uid < 1
...
export const loadMailSyncState = async (
    ...
    try {
        ...
        const [metadataRows]: any = await connection.query(
        ...
        const payloadLengths = [
            ['known_items', metadata.known_items_bytes, MAX_MAIL_SYNC_KNOWN_ITEMS_BYTES],
            ['last_commands', metadata.last_commands_bytes, MAX_MAIL_SYNC_COMMANDS_BYTES],
            ['last_response', metadata.last_response_bytes, MAX_MAIL_SYNC_REPLAY_BYTES],
        ] as const;
        let payloadBytes = 0;
        for (const [field, rawBytes, maximum] of payloadLengths) {
            ...
            if (bytes > maximum) throw new MailSyncStateError(`Mail sync state ${field} is too large`);
            payloadBytes += bytes;
        }
        if (payloadBytes > MAX_MAIL_SYNC_ROW_BYTES) {
            throw new MailSyncStateError('Mail sync state row is too large');
        }
        const [payloadRows]: any = await connection.query(
            `SELECT known_items, last_commands, last_response
        ...
        if (lastResponse && lastResponse.length > MAX_MAIL_SYNC_REPLAY_BYTES) {
        ...
        if (!Number.isFinite(updatedAt.getTime()) || Date.now() - updatedAt.getTime() > MAIL_SYNC_STATE_TTL_MS) {
        ...
        const knownItems = parseMailSyncKnownItems(row.known_items);
        const lastCommands = parseCommands(row.last_commands);
        assertMailSyncRowBound(
            JSON.stringify(knownItems),
            JSON.stringify(lastCommands),
            lastResponse,
        );
        const result: StoredMailSyncState = {
            scopeHash,
...
export const deleteMailSyncState = async (
    username: string,
    deviceId: string,
    collectionId: string,
): Promise<void> => {
    ...
    const scopeHash = mailSyncScopeHash(username, deviceId, collectionId);
    await pool.query(
        'DELETE FROM eas_mail_sync_states WHERE scope_hash = ? AND username = ? AND device_id = ? AND collection_id = ?',
...
export const saveMailSyncState = async (state: StoredMailSyncState): Promise<void> => {
    await ensureEasMailSyncSchema();
    if (state.scopeHash !== mailSyncScopeHash(state.username, state.deviceId, state.collectionId)
    ...
    if (Object.keys(state.knownItems).length > MAX_MAIL_SYNC_KNOWN_ITEMS
        || Buffer.byteLength(knownItems, 'utf8') > MAX_MAIL_SYNC_KNOWN_ITEMS_BYTES
        || state.lastCommands.length > 512 || Buffer.byteLength(commands, 'utf8') > MAX_MAIL_SYNC_COMMANDS_BYTES
        || state.lastCommands.some(command => !Number.isSafeInteger(command.uid) || command.uid < 1
            || !['Add', 'Change', 'Delete', 'SoftDelete'].includes(command.type)
            || command.isRead !== undefined && ![0, 1].includes(command.isRead))
        || (state.lastResponse && state.lastResponse.length > MAX_MAIL_SYNC_REPLAY_BYTES)) {
        throw new MailSyncStateError('Mail sync state exceeds its storage bound');
    }
    parseMailSyncKnownItems(knownItems);
    ...
    const connection = await pool.getConnection();
    const quotaLock = `oms-mail-${createHash('sha256').update(state.username).digest('hex').slice(0, 47)}`;
    ...
    try {
        ...
        const [partnershipRows]: any = await connection.query(
            'SELECT COUNT(*) AS count FROM eas_mail_sync_states WHERE username = ? AND scope_hash <> ?',
            [state.username, state.scopeHash],
        );
        if (Number(partnershipRows[0]?.count || 0) >= MAX_MAIL_SYNC_PARTNERSHIPS_PER_USER) {
        ...
        const [byteRows]: any = await connection.query(
            `SELECT COALESCE(SUM(OCTET_LENGTH(known_items) + OCTET_LENGTH(last_commands)
             ...
             FROM eas_mail_sync_states WHERE username = ? AND scope_hash <> ?`,
            [state.username, state.scopeHash],
        );
        if (Number(byteRows[0]?.bytes || 0) + rowBytes > MAX_MAIL_SYNC_USER_BYTES) {
...
export async function withMailSyncScopeLock<T>(scopeHash: string, operation: () => Promise<T>): Promise<T> {
    if (!/^[0-9a-f]{64}$/.test(scopeHash)) throw new MailSyncStateError('Mail sync scope lock identity is invalid');
    const connection = await pool.getConnection();
    ...
    try {
        ...
        if (value === 1 || value === '1') acquired = true;
        else if (value === 0 || value === '0') throw new MailSyncStateError('Mail sync scope lock was unavailable');
        else {
            connectionUsable = false;
