...
export const ACTIVE_SYNC_PING_MIN_HEARTBEAT_SECONDS = 60;
export const ACTIVE_SYNC_PING_MAX_HEARTBEAT_SECONDS = 900;
export const ACTIVE_SYNC_PING_MAX_FOLDERS = 32;
export const ACTIVE_SYNC_PING_MAX_REQUEST_BYTES = 32 * 1024;
export const ACTIVE_SYNC_PING_MAX_CACHED_CONFIGS = 4096;
export const ACTIVE_SYNC_PING_MAX_CACHED_CONFIGS_PER_OWNER = 64;
export const ACTIVE_SYNC_PING_CONFIG_TTL_MS = 24 * 60 * 60 * 1000;
export const ACTIVE_SYNC_PING_MAX_ACTIVE_WAITS = 64;
export const ACTIVE_SYNC_PING_MAX_ACTIVE_WAITS_PER_OWNER = 8;
export const ACTIVE_SYNC_PING_MAX_ACTIVE_WAITS_PER_SCOPE = 2;
export const ACTIVE_SYNC_PING_POLL_INTERVAL_MS = 15_000;
...
export type ActiveSyncPingResolvedFolder =
    | { id: string; className: 'Email'; kind: 'mail'; folderPath: string }
    | { id: 'contacts'; className: 'Contacts'; kind: 'contacts' }
    | { id: string; className: 'Calendar'; kind: 'calendar'; calendarId: number };

export type ActiveSyncPingFolderResolution =
    | { ok: true; folders: ActiveSyncPingResolvedFolder[] }
    | { ok: false; response: ActiveSyncPingResponse };

export type ActiveSyncPingResponse =
    | { status: '1' | '3' | '4' | '7' | '8' | '101' | '102' | '103' | '108' | '109' | '130' | '138' }
    | { status: '2'; folders: string[] }
    | { status: '5'; heartbeatSeconds: number }
    | { status: '6'; maxFolders: number };

export type ActiveSyncPingParseResult =
    | { ok: true; config: ActiveSyncPingConfig }
    | { ok: false; response: ActiveSyncPingResponse };

interface ActiveSyncPingCacheEntry {
    config: ActiveSyncPingConfig;
    expiresAt: number;
    ownerHash: string;
}

const cloneConfig = (config: ActiveSyncPingConfig): ActiveSyncPingConfig => ({
    heartbeatSeconds: config.heartbeatSeconds,
    folders: config.folders.map(folder => ({ ...folder })),
});

const validConfig = (config: ActiveSyncPingConfig): boolean => {
    if (!Number.isInteger(config.heartbeatSeconds)
        || config.heartbeatSeconds < ACTIVE_SYNC_PING_MIN_HEARTBEAT_SECONDS
        || config.heartbeatSeconds > ACTIVE_SYNC_PING_MAX_HEARTBEAT_SECONDS
        || !Array.isArray(config.folders) || config.folders.length < 1
        || config.folders.length > ACTIVE_SYNC_PING_MAX_FOLDERS) return false;
    ...
    for (const folder of config.folders) {
        if (!folder || typeof folder.id !== 'string'
            ...
            || seen.has(folder.id)) return false;
        seen.add(folder.id);
    }
    return true;
};

const pingScopeIdentity = (owner: string, deviceId: string): { key: string; ownerHash: string } | null => {
    if (typeof owner !== 'string' || typeof deviceId !== 'string'
        ...
        || /[\u0000-\u001f\u007f]/.test(owner + deviceId)) return null;
    const ownerHash = createHash('sha256').update(owner).digest('hex');
    const key = createHash('sha256').update(ownerHash).update('\0').update(deviceId).digest('hex');
    return { key, ownerHash };
};

export const activeSyncPingScopePrefix = (owner: string, deviceId: string): string =>
    pingScopeIdentity(owner, deviceId)?.key.slice(0, 12) || 'invalid';

export class ActiveSyncPingConfigCache {
    private readonly entries = new Map<string, ActiveSyncPingCacheEntry>();
    ...
    constructor(options: {
        ...
        this.maxEntries = options.maxEntries ?? ACTIVE_SYNC_PING_MAX_CACHED_CONFIGS;
        this.maxEntriesPerOwner = options.maxEntriesPerOwner
            ?? Math.min(ACTIVE_SYNC_PING_MAX_CACHED_CONFIGS_PER_OWNER, this.maxEntries);
        this.ttlMs = options.ttlMs ?? ACTIVE_SYNC_PING_CONFIG_TTL_MS;
    ...
    set(owner: string, deviceId: string, config: ActiveSyncPingConfig): boolean {
        const scope = pingScopeIdentity(owner, deviceId);
        if (!scope || !validConfig(config)) return false;
        const now = this.now();
        this.pruneExpired(now);
    ...
    delete(owner: string, deviceId: string): void {
        const scope = pingScopeIdentity(owner, deviceId);
        if (scope) this.entries.delete(scope.key);
    }
}

export class ActiveSyncPingAbortedError extends Error {
...
export class ActiveSyncPingSupersededError extends ActiveSyncPingAbortedError {
    constructor() {
...
interface ActiveSyncPingActiveWait {
    scopeKey: string;
    ownerHash: string;
    token: symbol;
...
export class ActiveSyncPingWaitRegistry {
    private readonly waits = new Map<symbol, ActiveSyncPingActiveWait>();
    private readonly currentByScope = new Map<string, symbol>();
    private readonly latestActivatedGenerationByScope = new Map<string, number>();
    private nextGeneration = 1;
    private readonly maxActive: number;
    private readonly maxActivePerOwner: number;
    private readonly maxActivePerScope: number;
    ...
    constructor(options: {
        ...
        this.maxActive = options.maxActive ?? ACTIVE_SYNC_PING_MAX_ACTIVE_WAITS;
        this.maxActivePerOwner = options.maxActivePerOwner
            ?? Math.min(ACTIVE_SYNC_PING_MAX_ACTIVE_WAITS_PER_OWNER, this.maxActive);
        this.maxActivePerScope = options.maxActivePerScope
            ?? Math.min(ACTIVE_SYNC_PING_MAX_ACTIVE_WAITS_PER_SCOPE, this.maxActivePerOwner);
    ...
    reserve(owner: string, deviceId: string): ActiveSyncPingPreflightReservation | null {
        ...
        if (this.waits.size >= this.maxActive
            ...
            || scopeWaits >= this.maxActivePerScope) return null;
        ...
        const release = () => {
            ...
            if (this.currentByScope.get(scope.key) === token) this.currentByScope.delete(scope.key);
            if (!Array.from(this.waits.values()).some(wait => wait.scopeKey === scope.key)) {
                this.latestActivatedGenerationByScope.delete(scope.key);
            }
        };
        const abort = (reason?: Error) => controller.abort(reason ?? new ActiveSyncPingAbortedError());
        return {
    ...
    abortAll(reason = new ActiveSyncPingAbortedError('ActiveSync Ping service is stopping')): void {
        for (const wait of this.waits.values()) wait.controller.abort(reason);
    }
}
...
export function activeSyncPingHasAdditions(
    knownItemIds: string[],
    ...
    for (const id of currentItemIds) {
        ...
        current.add(id);
        if (!known.has(id)) hasAdditions = true;
    }
    return hasAdditions;
}

export function activeSyncPingMailCursorNeedsSnapshot(
    syncModseq: string,
    liveModseq: string,
    lastProbedModseq?: string,
): boolean {
...
export async function evaluateActiveSyncPingChanges(
...
): Promise<ActiveSyncPingPollResult> {
    if (signal?.aborted) throw new ActiveSyncPingAbortedError();
    ...
    const byFolder = new Map<string, ActiveSyncPingFolderSnapshot>();
    ...
    for (const folder of folders) {
        ...
        if (!snapshot) throw new Error('ActiveSync Ping snapshot folder is unexpected');
        if (!snapshot.exists) return { kind: 'hierarchy' };
        if (!snapshot.initialized || snapshot.hasAdditions) changed.push(folder.id);
    }
...
const defaultPingScheduler: ActiveSyncPingScheduler = {
    now: () => performance.now(),
...
export function startActiveSyncPingWait(options: {
    heartbeatSeconds: number;
    folders: ActiveSyncPingResolvedFolder[];
    poll: (
        folders: ActiveSyncPingResolvedFolder[],
    ...
    const pollIntervalMs = options.pollIntervalMs ?? ACTIVE_SYNC_PING_POLL_INTERVAL_MS;
    if (!Number.isInteger(options.heartbeatSeconds)
        || options.heartbeatSeconds < ACTIVE_SYNC_PING_MIN_HEARTBEAT_SECONDS
        || options.heartbeatSeconds > ACTIVE_SYNC_PING_MAX_HEARTBEAT_SECONDS
        || !Array.isArray(options.folders) || options.folders.length < 1
        || options.folders.length > ACTIVE_SYNC_PING_MAX_FOLDERS
    ...
    const response = new Promise<ActiveSyncPingResponse>((resolve, reject) => {
        let settled = false;
        const maybeDrain = () => {
            if (responseSettled && inFlightPolls === 0
                && pollTimer === null && deadlineTimer === null) resolveDrained();
        };
        const cleanup = () => {
        ...
        const finish = (response: ActiveSyncPingResponse) => {
            if (settled) return;
            settled = true;
            cleanup();
            resolve(response);
        ...
        const poll = async () => {
            ...
            if (remaining <= pollIntervalMs) return;
...
export function waitForActiveSyncPing(
    options: Parameters<typeof startActiveSyncPingWait>[0],
): Promise<ActiveSyncPingResponse> {
    return startActiveSyncPingWait(options).response;
}

const hasContent = (node: WbxmlNode): boolean =>
    node.content !== undefined && node.content !== null;

const isLeaf = (node: WbxmlNode, tag: string): boolean =>
    node.tag === tag && node.page === 13 && hasContent(node) && !node.children?.length;

const safeText = (node: WbxmlNode, maxBytes: number): string | null => {
    if (typeof node.content !== 'string') return null;
    if (Buffer.byteLength(node.content, 'utf8') < 1 || Buffer.byteLength(node.content, 'utf8') > maxBytes) return null;
    if (/[\u0000-\u001f\u007f]/.test(node.content)) return null;
    return node.content;
};

const parseHeartbeat = (node: WbxmlNode): ActiveSyncPingParseResult | number => {
    ...
    if (negative || digits.length < 2
        || (digits.length === 2 && digits < String(ACTIVE_SYNC_PING_MIN_HEARTBEAT_SECONDS))) {
        return {
            ok: false,
            response: { status: '5', heartbeatSeconds: ACTIVE_SYNC_PING_MIN_HEARTBEAT_SECONDS },
        };
    }
    if (digits.length > 3
        || (digits.length === 3 && digits > String(ACTIVE_SYNC_PING_MAX_HEARTBEAT_SECONDS))) {
        return {
            ok: false,
            response: { status: '5', heartbeatSeconds: ACTIVE_SYNC_PING_MAX_HEARTBEAT_SECONDS },
        };
    }
    return Number(digits);
};

const parseFolders = (node: WbxmlNode): ActiveSyncPingParseResult | ActiveSyncPingFolder[] => {
    if (node.tag !== 'Folders' || node.page !== 13 || hasContent(node)
        || !Array.isArray(node.children) || node.children.length < 1) {
        return { ok: false, response: { status: '4' } };
    }
    const folders: ActiveSyncPingFolder[] = [];
    const seen = new Set<string>();
    for (const folder of node.children) {
        ...
        seen.add(id);
        folders.push({ id, className: className as ActiveSyncPingFolderClass });
    }
    if (folders.length > ACTIVE_SYNC_PING_MAX_FOLDERS) {
        return {
            ok: false,
...
export function parseActiveSyncPingRequest(
    decoded: WbxmlNode | null,
    cached?: ActiveSyncPingConfig | null,
): ActiveSyncPingParseResult {
    if (!decoded) {
        return cached
            ? {
                ok: true,
    ...
    if (decoded.children.length < 1) return { ok: false, response: { status: '3' } };
    ...
    if (heartbeatNodes.length === 1) {
        ...
        if (typeof heartbeat !== 'number') return heartbeat;
    ...
    if (folderNodes.length === 1) {
        ...
        if (!Array.isArray(parsedFolders)) return parsedFolders;
        folders = parsedFolders;
    }

    if (!heartbeatSeconds || !folders?.length) return { ok: false, response: { status: '3' } };
    return {
        ok: true,
...
const validResolvedFolder = (folder: ActiveSyncPingResolvedFolder): boolean => {
...
export function resolveActiveSyncPingFolders(
    requested: ActiveSyncPingFolder[],
    available: ActiveSyncPingResolvedFolder[],
): ActiveSyncPingFolderResolution {
    const byId = new Map<string, ActiveSyncPingResolvedFolder>();
    ...
    const folders: ActiveSyncPingResolvedFolder[] = [];
    for (const requestedFolder of requested) {
        const resolved = byId.get(requestedFolder.id);
        if (!resolved) return { ok: false, response: { status: '7' } };
...
export function activeSyncPingResponseNode(response: ActiveSyncPingResponse): WbxmlNode {
    return {
        tag: 'Ping',
        page: 13,
        children: [
