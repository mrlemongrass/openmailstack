...
exports.activeSyncSettingsResponseNode = void 0;
const protocolError = () => ({
    tag: 'Settings',
    page: 18,
    children: [{ tag: 'Status', page: 18, content: '2' }],
});
const hasNoContent = (node) => node.content === undefined;
const isOofGet = (request) => {
    if (!request || request.tag !== 'Settings' || request.page !== 18
        || !hasNoContent(request) || request.children.length !== 1)
        return false;
    const oof = request.children[0];
    if (oof.tag !== 'Oof' || oof.page !== 18 || !hasNoContent(oof)
        || oof.children.length !== 1)
        return false;
...
const activeSyncSettingsResponseNode = (request) => {
    if (!isOofGet(request))
...
exports.activeSyncSettingsResponseNode = activeSyncSettingsResponseNode;
//# sourceMappingURL=eas-settings.js.map