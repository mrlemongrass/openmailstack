...
exports.projectAvailabilitySlot = projectAvailabilitySlot;
...
const localPartsAt = (instant, timeZone) => {
...
const possibleInstants = (localDate, minuteOfDay, timeZone) => {
...
const assertMinute = (name, value, allowEndOfDay = false) => {
...
const validateRequest = (request) => {
    ...
    if (request.now && !Number.isFinite(request.now.getTime()))
        throw new Error('now must be a valid date');
    formatterFor(request.timeZone).format(request.rangeStart);
    if (request.rangeStart.getTime() >= request.rangeEnd.getTime())
    ...
    for (const window of request.windows) {
        ...
        if (window.startMinute >= window.endMinute)
            throw new Error('availability windows must not cross midnight');
    }
    for (const override of request.overrides || []) {
        ...
        for (const window of override.windows) {
            ...
            if (window.startMinute >= window.endMinute)
                throw new Error('availability override windows must not cross midnight');
        }
    }
    for (const interval of request.busy || []) {
        ...
        if (interval.start.getTime() >= interval.end.getTime())
            throw new Error('busy interval start must be before end');
    }
};
function calculateAvailability(request) {
    ...
    while (localDate <= finalLocalDate) {
        ...
        for (const window of windows) {
            for (let startMinute = window.startMinute; startMinute + request.durationMinutes <= window.endMinute; startMinute += request.intervalMinutes) {
                for (const start of possibleInstants(localDate, startMinute, request.timeZone)) {
                    const startMs = start.getTime();
                    const endMs = startMs + durationMs;
                    if (startMs < rangeStart || endMs > rangeEnd || startMs < earliestStart)
                        continue;
                    ...
                    const endsAtLocalMidnight = window.endMinute === 1440
                        ...
                        && endLocalMinute === 0;
                    if (!endsAtLocalMidnight && (endDate !== localDate || endLocalMinute > window.endMinute))
                        continue;
                    const occupiedStart = startMs - bufferBeforeMs;
                    const occupiedEnd = endMs + bufferAfterMs;
                    if (busy.some((interval) => overlaps(occupiedStart, occupiedEnd, interval.start, interval.end)))
                        continue;
...
function projectAvailabilitySlot(slot, timeZone) {
