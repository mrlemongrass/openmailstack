...
exports.SchedulerPhase2Store = void 0;
...
class SchedulerPhase2Store {
    pool;
    scheduler;
    constructor(pool, scheduler) {
        this.pool = pool;
        this.scheduler = scheduler;
    }
    async createPoll(username, input) {
        ...
        if (event.visibility === 'private')
            throw new Error('Meeting polls require a listed or unlisted event type');
        const title = String(input.title || event.title).trim().slice(0, 160);
        if (!title)
            throw new Error('Poll title is required');
        const starts = Array.isArray(input.starts) ? [...new Set(input.starts.map(String))].map(value => new Date(value)) : [];
        ...
        const available = await this.scheduler.listSlots(entitlement.handle, event.slug, new Date(starts[0].getTime() - 1), new Date(starts.at(-1).getTime() + event.durationMinutes * 60_000 + 1));
        const availableStarts = new Set(available.map(slot => slot.start.getTime()));
        if (starts.some(start => !availableStarts.has(start.getTime())))
            throw new Error('Every poll option must currently be an available time');
        const id = crypto_1.default.randomUUID();
        const token = (0, phase1_1.createSchedulerToken)();
        const connection = await this.pool.getConnection();
        try {
            await connection.beginTransaction();
            await connection.query(`INSERT INTO scheduler_meeting_polls (id,tenant_key,owner_username,event_type_id,title,token_hash)
                 VALUES (?,?,?,?,?,?)`, [id, entitlement.tenantKey, username, event.id, title, (0, phase1_1.schedulerTokenHash)(token)]);
    ...
    async listPolls(username) {
        await this.scheduler.requireOwner(username);
        const [rows] = await this.pool.query(`SELECT p.id,p.title,p.status,p.event_type_id,p.finalized_option_id,e.title AS event_title,
    ...
    async getPublicPoll(token) {
        const [rows] = await this.pool.query(`SELECT p.id,p.title,p.status,e.title AS event_title,e.require_email_verification,m.display_name,
             ...
             GROUP BY p.id,o.id ORDER BY o.position`, [(0, phase1_1.schedulerTokenHash)(token)]);
        if (!rows.length)
            return null;
        return {
            ...
            options: rows.map((row) => ({ id: row.option_id, start: utcDate(row.slot_start_utc), end: utcDate(row.slot_end_utc), votes: Number(row.votes) })),
        };
    }
    async votePoll(token, input) {
        const poll = await this.getPublicPoll(token);
        if (!poll || poll.status !== 'open')
            throw new Error('Meeting poll is not available');
        ...
        if (!voterName || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(voterEmail))
            throw new Error('A valid name and email are required');
        if (!optionIds.length || optionIds.some(id => !poll.options.some((option) => option.id === id)))
            throw new Error('Choose at least one valid poll option');
        const [policyRows] = await this.pool.query(`SELECT p.event_type_id,e.guest_allow_list,e.guest_deny_list,e.require_email_verification
             FROM scheduler_meeting_polls p JOIN scheduler_event_types e ON e.id=p.event_type_id WHERE p.id=? LIMIT 1`, [poll.id]);
        const policy = policyRows[0];
        const rules = (value) => { try {
        ...
        (0, phase1_1.assertSchedulerGuestEligible)(voterEmail, rules(policy.guest_allow_list), rules(policy.guest_deny_list));
        let verification = null;
        if (Number(policy.require_email_verification) === 1) {
            const challengeId = String(input.verificationChallengeId || '');
            const [verificationRows] = await this.pool.query(`SELECT * FROM scheduler_email_verifications WHERE id=? AND event_type_id=? AND booker_email=?
                 AND used_at IS NULL AND expires_at>UTC_TIMESTAMP(3) AND attempts<5 LIMIT 1`, [challengeId, policy.event_type_id, voterEmail]);
        ...
        try {
            await connection.beginTransaction();
            if (verification) {
                const [locked] = await connection.query(`SELECT id FROM scheduler_email_verifications WHERE id=? AND used_at IS NULL
            ...
            for (const optionId of optionIds) {
                await connection.query(`INSERT INTO scheduler_poll_votes (id,poll_id,option_id,voter_name,voter_email) VALUES (?,?,?,?,?)`, [crypto_1.default.randomUUID(), poll.id, optionId, voterName, voterEmail]);
            }
            await connection.commit();
        }
        catch (error) {
    ...
    async requestPollVerification(token, email) {
        const [rows] = await this.pool.query(`SELECT m.public_handle,e.slug FROM scheduler_meeting_polls p
             JOIN scheduler_event_types e ON e.id=p.event_type_id
             JOIN scheduler_mailbox_entitlements m ON m.username=p.owner_username
             WHERE p.token_hash=? AND p.status='open' LIMIT 1`, [(0, phase1_1.schedulerTokenHash)(token)]);
        if (!rows.length)
            throw new Error('Meeting poll is not available');
        return this.scheduler.requestEmailVerification(rows[0].public_handle, rows[0].slug, email);
    }
    async finalizePoll(username, pollId, optionId) {
        await this.scheduler.requireOwner(username);
        const [pollRows] = await this.pool.query(`SELECT p.*,e.capacity,e.max_additional_guests,m.public_handle,CAST(o.slot_start AS CHAR) AS slot_start_utc
             FROM scheduler_meeting_polls p JOIN scheduler_event_types e ON e.id=p.event_type_id
             JOIN scheduler_mailbox_entitlements m ON m.username=p.owner_username
        ...
        if (!poll)
            throw new Error('Meeting poll not found');
        const [lockResult] = await this.pool.query("UPDATE scheduler_meeting_polls SET status='closed' WHERE id=? AND status='open'", [pollId]);
        if (!lockResult.affectedRows)
            throw new Error('Meeting poll is already finalized or closed');
        try {
            const [voterRows] = await this.pool.query(`SELECT voter_name,voter_email FROM scheduler_poll_votes
            ...
            const additional = voterRows.slice(1, Number(poll.max_additional_guests || 0) + 1)
                .map((row) => ({ name: row.voter_name, email: row.voter_email }));
    ...
    async exportOwnerData(username) {
        const entitlement = await this.scheduler.requireOwner(username);
    ...
    async exportBookingsCsv(username) {
        ...
        return [header.map(csvCell).join(','), ...bookings.map((booking) => [
    ...
    async importOwnerData(username, sourceValue, payload) {
        const source = (0, phase2_1.normalizeImportSource)(sourceValue);
        const entitlement = await this.scheduler.requireOwner(username);
        const rawEvents = source === 'openmailstack' ? payload?.events : source === 'calendly' ? payload?.event_types : payload?.eventTypes;
        if (!Array.isArray(rawEvents) || rawEvents.length > 100)
            throw new Error('Import must contain between 0 and 100 event types');
        let imported = 0;
        let skipped = 0;
        const errors = [];
        ...
        const id = crypto_1.default.randomUUID();
        await this.pool.query(`INSERT INTO scheduler_import_runs (id,tenant_key,owner_username,source,imported_events,skipped_events) VALUES (?,?,?,?,?,?)`, [id, entitlement.tenantKey, username, source, imported, skipped]);
        return { id, source, imported, skipped, errors: errors.slice(0, 20) };
    }
}
exports.SchedulerPhase2Store = SchedulerPhase2Store;
//# sourceMappingURL=phase2-store.js.map