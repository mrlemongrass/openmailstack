...
exports.SchedulerSlotHoldRepository = exports.SlotUnavailableError = void 0;
...
const assertInput = (input) => {
    ...
    if (input.slotStart.getTime() >= input.slotEnd.getTime())
        throw new Error('slotStart must be before slotEnd');
    if (!Number.isInteger(input.capacity) || input.capacity <= 0)
        throw new Error('capacity must be a positive integer');
    const seats = input.seats ?? 1;
    if (!Number.isInteger(seats) || seats <= 0 || seats > input.capacity)
        throw new Error('seats must be between 1 and capacity');
...
const parseMysqlUtc = (value) => new Date(`${value.replace(' ', 'T')}Z`);
...
class SchedulerSlotHoldRepository {
    pool;
    constructor(pool) {
        this.pool = pool;
    }
    ...
    async acquireInTransaction(connection, input) {
        const seats = input.seats ?? 1;
        const now = input.now || new Date();
        const expiresAt = new Date(now.getTime() + input.ttlSeconds * 1000);
        const key = [input.tenantKey, input.eventTypeKey, input.hostUsername, mysqlDate(input.slotStart)];
        const [existingRows] = await connection.query(`SELECT hold_token, tenant_key, event_type_key, host_username,
             ...
             LIMIT 1
             FOR UPDATE`, [input.tenantKey, input.idempotencyKey]);
        if (existingRows.length > 0) {
            const existing = mapHold(existingRows[0]);
            if (existing.status === 'released') {
                await connection.query('DELETE FROM scheduler_slot_holds WHERE hold_token = ? AND status = ?', [existing.token, 'released']);
            }
            else {
        ...
        await connection.query(`INSERT INTO scheduler_slot_inventory
                (tenant_key, event_type_key, host_username, slot_start, slot_end, capacity, held_seats, confirmed_seats)
             VALUES (?, ?, ?, ?, ?, ?, 0, 0)
             ON DUPLICATE KEY UPDATE updated_at = CURRENT_TIMESTAMP(3)`, [...key, mysqlDate(input.slotEnd), input.capacity]);
        const [inventoryRows] = await connection.query(`SELECT slot_end = ? AS slot_end_matches, capacity, held_seats, confirmed_seats
             FROM scheduler_slot_inventory
             WHERE tenant_key = ? AND event_type_key = ? AND host_username = ? AND slot_start = ?
             FOR UPDATE`, [mysqlDate(input.slotEnd), ...key]);
        const inventory = inventoryRows[0];
        if (!inventory)
            throw new Error('Failed to lock scheduler slot inventory');
        ...
        const [expiredRows] = await connection.query(`SELECT COALESCE(SUM(seats), 0) AS expired_seats
             FROM scheduler_slot_holds
             WHERE tenant_key = ? AND event_type_key = ? AND host_username = ? AND slot_start = ?
               AND status = 'held' AND expires_at <= ?`, [...key, mysqlDate(now)]);
        const expiredSeats = Number(expiredRows[0]?.expired_seats || 0);
        if (expiredSeats > 0) {
            await connection.query(`UPDATE scheduler_slot_holds SET status = 'expired'
                 WHERE tenant_key = ? AND event_type_key = ? AND host_username = ? AND slot_start = ?
                   AND status = 'held' AND expires_at <= ?`, [...key, mysqlDate(now)]);
            await connection.query(`UPDATE scheduler_slot_inventory
                 SET held_seats = GREATEST(held_seats - ?, 0)
                 WHERE tenant_key = ? AND event_type_key = ? AND host_username = ? AND slot_start = ?`, [expiredSeats, ...key]);
        ...
        await connection.query(`INSERT INTO scheduler_slot_holds
        ...
        await connection.query(`UPDATE scheduler_slot_inventory SET held_seats = held_seats + ?
             WHERE tenant_key = ? AND event_type_key = ? AND host_username = ? AND slot_start = ?`, [seats, ...key]);
        return {
            token,
...
exports.SchedulerSlotHoldRepository = SchedulerSlotHoldRepository;
//# sourceMappingURL=slot-holds.js.map