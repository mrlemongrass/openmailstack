...
const replaceTitle = (template: string, title: string): string => template.replaceAll('{{event.title}}', title);
...
export async function runSchedulerJobCycle(
    ...
    for (const job of jobs) {
        ...
        let mail: SchedulerReminderMail;
        try {
        ...
        try {
            ...
            if (error?.disposition !== 'safe_to_retry') throw error;
...
export class SchedulerWorkflowRepository {
    ...
    async publishVersion(workflowId: string, createdBy: string, value: unknown): Promise<{ id: string; version: number }> {
        ...
        try {
            await connection.beginTransaction();
            const [rows]: any = await connection.query(
                'SELECT tenant_key, owner_username, current_version FROM scheduler_workflows WHERE id=? FOR UPDATE',
                [workflowId],
            );
            ...
            for (const [index, step] of definition.steps.entries()) {
                await connection.query(
                    `INSERT INTO scheduler_workflow_steps
                     ...
                     VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [crypto.randomUUID(), rows[0].tenant_key, versionId, index + 1, step.action, step.delaySeconds, JSON.stringify(step.config)],
                );
            }
    ...
    async captureForBooking(db: Queryable, input: SchedulerBookingWorkflowInput): Promise<number> {
        const [rows]: any = await db.query(
            `SELECT w.id AS workflow_id, v.id AS version_id, v.version, v.trigger_offset_seconds,
    ...
    async cancelForBooking(db: Queryable, tenantKey: string, bookingId: string): Promise<void> {
        await db.query(
            `UPDATE scheduler_delivery_attempts a
             JOIN scheduler_jobs j ON j.id=a.job_id
...
export class SchedulerJobRepository implements SchedulerJobStore {
    ...
    async claimBatch(workerId: string, limit: number, _leaseUntil: Date): Promise<SchedulerJobClaim[]> {
        ...
        try {
            ...
            const [rows]: any = await connection.query(
                `SELECT j.id, j.idempotency_key, j.payload, j.attempts, s.config
