...
exports.UniversalOutboundFingerprintConflictError = UniversalOutboundFingerprintConflictError;
const schemaPromises = new WeakMap();
const ensureOutboundRegistrySchema = async (db) => {
...
exports.ensureOutboundRegistrySchema = ensureOutboundRegistrySchema;
const isDuplicateKeyError = (error) => (String(error?.code || '') === 'ER_DUP_ENTRY' || Number(error?.errno) === 1062);
const transactionConnection = async (db) => {
...
const reserveUniversalOutbound = async (db, reservation) => {
    ...
    const existing = await (0, exports.findUniversalOutboundIdentity)(db, reservation.username, { idempotencyKey: reservation.idempotencyKey });
    if (!existing)
        throw new Error('The existing outbound reservation is unavailable');
...
const abortUniversalOutboundReservation = async (db, id, username) => {
    ...
    try {
        await connection.beginTransaction();
        const [rows] = await connection.query(`SELECT id, idempotency_key, request_fingerprint, submission_origin, submission_kind
             FROM scheduled_emails
             WHERE id = ? AND username = ? AND status = 'scheduled'
               AND submission_kind = 'scheduled' AND smtp_accepted_at IS NULL
             LIMIT 1 FOR UPDATE`, [id, username]);
        const row = rows?.[0];
        if (!row) {
            await connection.commit();
            return false;
        ...
        if (row.idempotency_key !== null) {
            ...
            const [registryResult] = await connection.query(`UPDATE outbound_submission_registry
                 ...
                 WHERE username = ? AND idempotency_key = ? AND submission_id = ?
                   ...
                   AND submission_origin = ?`, [replayDays, username, row.idempotency_key, id, row.request_fingerprint,
        ...
        const [deleteResult] = await connection.query(`DELETE FROM scheduled_emails
             WHERE id = ? AND username = ? AND status = 'scheduled'
               AND submission_kind = 'scheduled' AND smtp_accepted_at IS NULL`, [id, username]);
...
exports.abortUniversalOutboundReservation = abortUniversalOutboundReservation;
...
const backfillOutboundRegistry = async (db, batchSize = 100) => {
    const limit = Math.max(1, Math.min(1000, Math.trunc(batchSize)));
    await (0, exports.ensureOutboundRegistrySchema)(db);
    const [result] = await db.query(`INSERT IGNORE INTO outbound_submission_registry
            (username, idempotency_key, request_fingerprint, submission_id,
         ...
         ORDER BY s.id
         LIMIT ?`, [limit]);
    const [remainingRows] = await db.query(`SELECT 1
         FROM scheduled_emails s
         LEFT JOIN outbound_submission_registry r
           ON r.username = s.username AND r.idempotency_key = s.idempotency_key
         WHERE s.idempotency_key IS NOT NULL AND r.username IS NULL
...
exports.backfillOutboundRegistry = backfillOutboundRegistry;
const assertRegistryCoverage = async (db) => {
    const [mismatches] = await db.query(`SELECT s.id
         FROM scheduled_emails s
         LEFT JOIN outbound_submission_registry r
           ON r.username = s.username AND r.idempotency_key = s.idempotency_key
...
const compactHotRows = async (db, batchSize) => {
    const { connection, release } = await transactionConnection(db);
    try {
        await connection.beginTransaction();
        const [rows] = await connection.query(`SELECT s.id, s.username, s.idempotency_key, s.request_fingerprint,
             ...
             LIMIT ? FOR UPDATE SKIP LOCKED`, [batchSize]);
        let removed = 0;
        for (const row of rows || []) {
            ...
            const [registryResult] = await connection.query(`UPDATE outbound_submission_registry
                 ...
                 WHERE username = ? AND idempotency_key = ? AND submission_id = ?
                   ...
                   AND submission_origin = ?`, [row.status, row.last_error_code || null, row.terminal_at, row.smtp_accepted_at ? 1 : 0,
            ...
            const [deleteResult] = await connection.query(`DELETE FROM scheduled_emails
                 WHERE id = ? AND username = ? AND idempotency_key = ?
                   AND request_fingerprint = ? AND status = ?`, [row.id, row.username, row.idempotency_key, row.request_fingerprint, row.status]);
            if (Number(deleteResult?.affectedRows) !== 1) {
...
const compactUniversalOutbox = async (db, options) => {
    if (options.mode !== exports.OUTBOUND_COMPACTION_VERIFIED_MODE) {
    ...
    const [purgeResult] = await db.query(`UPDATE scheduled_emails s
    ...
    const [tombstoneResult] = await db.query(`DELETE r FROM outbound_submission_registry r
...
exports.compactUniversalOutbox = compactUniversalOutbox;
const projectMixedBasisInstant = (row, localField = 'send_at', utcAlias = 'send_at_utc') => {
    const value = row.idempotency_key === null ? row[localField] : (row[utcAlias] || row[localField]);
    const instant = value instanceof Date ? value : new Date(value);
    if (!Number.isFinite(instant.getTime()))
        throw new Error('Outbound schedule contains an invalid instant');
    return instant;
};
exports.projectMixedBasisInstant = projectMixedBasisInstant;
const selectMixedBasisDueRows = async (connection, limit, now) => {
    const boundedLimit = Math.max(1, Math.min(100, Math.trunc(limit)));
    const [legacyRows] = await connection.query(`SELECT scheduled_emails.*,
         ...
         LIMIT ? FOR UPDATE SKIP LOCKED`, [now, boundedLimit]);
    const [utcRows] = await connection.query(`SELECT scheduled_emails.*,
                DATE_FORMAT(COALESCE(available_at, send_at), '%Y-%m-%dT%H:%i:%s.000Z') AS due_at_utc,
                'utc' AS due_basis
         FROM scheduled_emails
         WHERE (idempotency_key IS NOT NULL OR status <> 'scheduled')
           ...
           AND (lease_expires_at IS NULL OR lease_expires_at <= UTC_TIMESTAMP())
         ORDER BY COALESCE(available_at, send_at), id
         LIMIT ? FOR UPDATE SKIP LOCKED`, [boundedLimit]);
...
exports.selectMixedBasisDueRows = selectMixedBasisDueRows;
//# sourceMappingURL=universal-outbox.js.map