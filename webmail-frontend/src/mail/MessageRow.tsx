...
export function MessageRow({
  ...
  selectionDisabled = false, forwardedRef,
}: MessageRowProps) {
  ...
  return (
    <div ref={forwardedRef}
      ...
      role={isScheduled || isDraft ? 'link' : undefined}
      tabIndex={isScheduled || isDraft ? 0 : undefined}
      aria-label={isScheduled
        ? `Open scheduled message ${message.subject || '(no subject)'}`
        : isDraft ? `Open draft ${message.subject || '(no subject)'}` : undefined}
      style={{
      ...
      onKeyDown={(e) => {
        ...
        if ((!isScheduled && !isDraft) || (e.key !== 'Enter' && e.key !== ' ')) return;
        e.preventDefault();
        onClick(message.uid);
      }}>
