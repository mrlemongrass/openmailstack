...
function BookingEvent({ profile, event, rootDefault = false, accessToken = '' }: { profile: SchedulerEntitlement; event: SchedulerEventType; rootDefault?: boolean; accessToken?: string }) {
  ...
  const submit = async (submitEvent: React.FormEvent) => {
    ...
    try {
      await createPublicBooking(profile.handle, event.slug, {
  ...
  if (confirmed) return <div className="public-scheduler-page"><PublicHeader /><main className="booking-confirmation"><div className="confirmation-mark"><Check size={28} /></div><h1>You're booked</h1><p>{event.title} with {profile.displayName || profile.username}</p><dl><div><dt>Date</dt><dd>{dateLabel(confirmed.start, timeZone)}</dd></div><div><dt>Time</dt><dd>{timeLabel(confirmed.start, timeZone)} · {event.durationMinutes} min</dd></div><div><dt>Time zone</dt><dd>{timeZone}</dd></div>{(event.questions || []).filter(question => bookingAnswers[question.id]).map(question => <div key={question.id}><dt>{question.label}</dt><dd>{bookingAnswers[question.id]}</dd></div>)}</dl><a className="btn btn-secondary confirmation-download" href={calendarDownload(profile, event, confirmed)} download={`${event.slug || 'booking'}.ics`}><Download size={16} /> Download calendar file</a><p className="confirmation-email">A calendar invitation and management links are on their way to {form.bookerEmail}.</p></main></div>;
