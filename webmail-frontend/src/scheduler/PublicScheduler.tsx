...
const dateLabel = (value: string, timeZone: string, locale?: string) => new Intl.DateTimeFormat(locale || [], { timeZone, weekday: 'short', month: 'short', day: 'numeric' }).format(new Date(value));
const timeLabel = (value: string, timeZone: string, locale?: string) => new Intl.DateTimeFormat(locale || [], { timeZone, hour: 'numeric', minute: '2-digit' }).format(new Date(value));
...
function BookingEvent({ profile, event, rootDefault = false, accessToken = '' }: { profile: SchedulerEntitlement; event: SchedulerEventType; rootDefault?: boolean; accessToken?: string }) {
  ...
  const [form, setForm] = useState({ bookerName: query.get('name')?.slice(0, 160) || '', bookerEmail: query.get('email')?.slice(0, 255) || '', bookerNotes: query.get('notes')?.slice(0, 4000) || '' });
  ...
  const grouped = useMemo(() => {
    ...
    slots.forEach(slot => { const key = dateLabel(slot.start, timeZone, event.locale); groups.set(key, [...(groups.get(key) || []), slot]); });
  ...
  const submit = async (submitEvent: React.FormEvent) => {
    ...
    try {
      ...
      if (selected.remainingSeats < seats) {
        ...
        setConfirmation({ slot: selected, status: 'waitlisted', seats, attendees });
        return;
      }
  ...
  useEffect(() => {
    if (confirmation && window.parent !== window) window.parent.postMessage({ type: 'oms.scheduler.booking.confirmed', status: confirmation.status }, '*');
  }, [confirmation]);
  if (confirmation) {
    const requested = confirmation.status === 'requested';
    const waitlisted = confirmation.status === 'waitlisted';
    return <div className="public-scheduler-page" style={{ '--scheduler-accent': event.publicAccentColor } as React.CSSProperties}>{query.get('embed') !== 'inline' && <PublicHeader />}<main className="booking-confirmation"><div className="confirmation-mark"><Check size={28} /></div><h1>{waitlisted ? 'You’re on the waitlist' : requested ? 'Request sent' : "You're booked"}</h1><p>{event.title} with {profile.displayName || profile.username}</p><dl><div><dt>Date</dt><dd>{dateLabel(confirmation.slot.start, timeZone, event.locale)}</dd></div><div><dt>Time</dt><dd>{timeLabel(confirmation.slot.start, timeZone, event.locale)} · {event.durationMinutes} min</dd></div><div><dt>Time zone</dt><dd>{timeZone}</dd></div>{recurrenceCount > 1 && <div><dt>Series</dt><dd>{recurrenceCount} weekly meetings</dd></div>}{event.capacity > 1 && <div><dt>Seats</dt><dd>{confirmation.seats}</dd></div>}{confirmation.attendees.map(attendee => <div key={attendee.email}><dt>Guest</dt><dd>{attendee.name || attendee.email}{attendee.name && ` · ${attendee.email}`}</dd></div>)}{(event.questions || []).filter(question => bookingAnswers[question.id]).map(question => <div key={question.id}><dt>{question.label}</dt><dd>{bookingAnswers[question.id]}</dd></div>)}</dl>{!requested && !waitlisted && recurrenceCount === 1 && <a className="btn btn-secondary confirmation-download" href={calendarDownload(profile, event, confirmation.slot)} download={`${event.slug || 'booking'}.ics`}><Download size={16} /> Download calendar file</a>}<p className="confirmation-email">{waitlisted ? `We will automatically promote your party and email ${form.bookerEmail} if enough seats open.` : requested ? `The time is reserved while ${profile.displayName || profile.username} reviews your request. We will email you with the decision.` : `Calendar invitations and management links are on their way to ${form.bookerEmail}.`}</p></main></div>;
  }
  return <div className="public-scheduler-page" lang={event.locale} style={{ '--scheduler-accent': event.publicAccentColor } as React.CSSProperties}>{query.get('embed') !== 'inline' && <PublicHeader />}<main className="public-booking-layout">
    <section className="public-event-summary">{!rootDefault && <Link to={`/scheduler/${profile.handle}`}><ArrowLeft size={16} /> {profile.displayName || profile.handle}</Link>}<h1>{event.title}</h1>{event.publicIntro && <p className="public-event-intro">{event.publicIntro}</p>}{event.description && <p>{event.description}</p>}<ul><li><Clock3 size={16} /> {event.durationMinutes} minutes</li>{event.locationLabel && <li><MapPin size={16} /> {event.locationLabel}</li>}<li><Globe2 size={16} /> {timeZone}</li></ul>{(event.privacyUrl || event.termsUrl) && <footer>{event.privacyUrl && <a href={event.privacyUrl} target="_blank" rel="noreferrer">Privacy</a>}{event.termsUrl && <a href={event.termsUrl} target="_blank" rel="noreferrer">Terms</a>}</footer>}</section>
    <section className="public-slot-picker"><div className="public-picker-heading"><h2>Select a time</h2><label><Globe2 size={15} /><select aria-label="Booking time zone" disabled={Boolean(event.lockedTimeZone)} value={timeZone} onChange={e => setTimeZone(e.target.value)}>{!timeZones.includes(timeZone) && <option value={timeZone}>{timeZone}</option>}{timeZones.map(zone => <option value={zone} key={zone}>{zone.replaceAll('_', ' ')}</option>)}</select></label></div>
      {error && <ErrorBanner error={error} />}{loading ? <p className="public-muted">Loading available times...</p> : grouped.length === 0 ? <p className="public-muted">No available times in the next 62 days.</p> : <div className="public-slot-days">{grouped.map(([day, daySlots]) => <div key={day}><h3>{day}</h3><div>{daySlots.map(slot => <button className={`${selected?.start === slot.start ? 'selected' : ''} ${slot.remainingSeats === 0 ? 'full' : ''}`} onClick={() => { if (selected?.start !== slot.start) { bookingAttemptKeyRef.current = crypto.randomUUID(); setSeats(1); setAttendees([]); } setSelected(slot); }} key={slot.start}><span>{timeLabel(slot.start, timeZone, event.locale)}</span>{slot.remainingSeats === 0 ? <small>Waitlist</small> : event.capacity > 1 && <small>{slot.remainingSeats} {slot.remainingSeats === 1 ? 'seat' : 'seats'} left</small>}</button>)}</div></div>)}</div>}
    </section>
    {selected && <form className="public-booking-form" onSubmit={submit}><h2>{selected.remainingSeats === 0 ? 'Join the waitlist' : 'Your details'}</h2><p>{dateLabel(selected.start, timeZone, event.locale)} at {timeLabel(selected.start, timeZone, event.locale)}</p><label>Name<input required autoComplete="name" value={form.bookerName} onChange={e => setForm({ ...form, bookerName: e.target.value })} /></label><label>Email<input required type="email" autoComplete="email" value={form.bookerEmail} onChange={e => { setForm({ ...form, bookerEmail: e.target.value }); setVerification({ challengeId: '', code: '', sentTo: '', sending: false, message: '' }); }} /></label>{event.requireEmailVerification && <div className="public-verification"><button type="button" className="btn btn-secondary" disabled={verification.sending || !form.bookerEmail.trim()} onClick={() => void sendVerification()}>{verification.sending ? 'Sending...' : verification.challengeId ? 'Send a new code' : 'Send verification code'}</button>{verification.challengeId && <label>Verification code<input required autoComplete="one-time-code" inputMode="text" maxLength={10} value={verification.code} onChange={e => setVerification({ ...verification, code: e.target.value })} /></label>}{verification.message && <small>{verification.message}</small>}</div>}{event.capacity > 1 && <label>Seats<select value={Math.max(seats, minimumSeats)} onChange={e => setSeats(Number(e.target.value))}>{Array.from({ length: seatLimit - minimumSeats + 1 }, (_, index) => index + minimumSeats).map(count => <option key={count} value={count}>{count}</option>)}</select></label>}{event.maxRecurrenceOccurrences > 1 && selected.remainingSeats > 0 && <label>Weekly meetings<select value={recurrenceCount} onChange={e => setRecurrenceCount(Number(e.target.value))}>{Array.from({ length: event.maxRecurrenceOccurrences }, (_, index) => index + 1).map(count => <option value={count} key={count}>{count === 1 ? 'One meeting' : `${count} weekly meetings`}</option>)}</select></label>}{event.maxAdditionalGuests > 0 && <fieldset className="public-attendees"><legend>Additional guests <small>Optional · up to {attendeeLimit} for this time</small></legend>{attendees.map((attendee, index) => <div className="public-attendee-row" key={index}><input aria-label={`Additional guest ${index + 1} name`} placeholder="Name" maxLength={160} value={attendee.name} onChange={e => setAttendees(current => current.map((item, itemIndex) => itemIndex === index ? { ...item, name: e.target.value } : item))} /><input aria-label={`Additional guest ${index + 1} email`} required type="email" placeholder="Email" value={attendee.email} onChange={e => setAttendees(current => current.map((item, itemIndex) => itemIndex === index ? { ...item, email: e.target.value } : item))} /><button type="button" onClick={() => setAttendees(current => current.filter((_, itemIndex) => itemIndex !== index))} aria-label={`Remove additional guest ${index + 1}`}>Remove</button></div>)}{attendees.length < attendeeLimit && <button type="button" className="public-add-attendee" onClick={() => { const nextCount = attendees.length + 2; setAttendees(current => [...current, { name: '', email: '' }]); setSeats(current => Math.max(current, nextCount)); }}>Add guest</button>}</fieldset>}{(event.questions || []).map(question => <label key={question.id}>{question.label}{question.required && <span className="public-required">Required</span>}{question.type === 'long_text' ? <textarea required={question.required} maxLength={2000} rows={3} value={bookingAnswers[question.id] || ''} onChange={e => setBookingAnswers({ ...bookingAnswers, [question.id]: e.target.value })} /> : question.type === 'select' ? <select required={question.required} value={bookingAnswers[question.id] || ''} onChange={e => setBookingAnswers({ ...bookingAnswers, [question.id]: e.target.value })}><option value="">Choose an option</option>{question.options.map(option => <option value={option} key={option}>{option}</option>)}</select> : <input required={question.required} maxLength={255} value={bookingAnswers[question.id] || ''} onChange={e => setBookingAnswers({ ...bookingAnswers, [question.id]: e.target.value })} />}</label>)}<label>Notes<textarea rows={3} maxLength={4000} value={form.bookerNotes} onChange={e => setForm({ ...form, bookerNotes: e.target.value })} /></label><button className="btn btn-primary" disabled={submitting || (event.requireEmailVerification && (!verification.challengeId || !verification.code.trim() || verification.sentTo !== form.bookerEmail.trim().toLowerCase()))}>{submitting ? 'Saving…' : selected.remainingSeats === 0 ? 'Join waitlist' : event.requiresConfirmation ? 'Request booking' : recurrenceCount > 1 ? 'Book series' : 'Confirm booking'}</button></form>}
  </main></div>;
}
...
export function PublicSchedulerPollPage() {
  ...
  const [form, setForm] = useState({ voterName: '', voterEmail: '', optionIds: [] as string[], verificationChallengeId: '', verificationCode: '' });
  const [status, setStatus] = useState('');
  const [error, setError] = useState('');
  useEffect(() => { getPublicPoll(token).then(result => setPoll(result.poll)).catch(err => setError(err instanceof Error ? err.message : 'Poll unavailable')); }, [token]);
  const submit = async (event: React.FormEvent) => {
    event.preventDefault(); setError('');
    try { await votePublicPoll(token, form); setStatus('Your availability has been saved.'); }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to save vote'); }
  };
  return <div className="public-scheduler-page"><PublicHeader /><main className="public-poll-page">{error && <ErrorBanner error={error} />}{!poll ? !error && <p>Loading poll…</p> : <><CalendarCheck size={30} /><h1>{poll.title}</h1><p>{poll.eventTitle} with {poll.hostName}</p>{poll.status !== 'open' ? <div className="scheduler-action-closed"><strong>This poll is {poll.status}.</strong><span>Voting is no longer available.</span></div> : <form onSubmit={submit}><label>Name<input required value={form.voterName} onChange={event => setForm({ ...form, voterName: event.target.value })} /></label><label>Email<input required type="email" value={form.voterEmail} onChange={event => setForm({ ...form, voterEmail: event.target.value, verificationChallengeId: '', verificationCode: '' })} /></label>{poll.requireEmailVerification && <div className="public-verification"><button type="button" className="btn btn-secondary" disabled={!form.voterEmail} onClick={async () => { try { const challenge = await requestPublicPollVerification(token, form.voterEmail); setForm({ ...form, verificationChallengeId: challenge.challengeId, verificationCode: '' }); setStatus('Verification code sent.'); } catch (err) { setError(err instanceof Error ? err.message : 'Unable to send code'); } }}>Send verification code</button>{form.verificationChallengeId && <label>Verification code<input required maxLength={10} autoComplete="one-time-code" value={form.verificationCode} onChange={event => setForm({ ...form, verificationCode: event.target.value })} /></label>}</div>}<fieldset><legend>Choose every time that works</legend>{poll.options.map(option => <label key={option.id}><input type="checkbox" checked={form.optionIds.includes(option.id)} onChange={event => setForm({ ...form, optionIds: event.target.checked ? [...form.optionIds, option.id] : form.optionIds.filter(id => id !== option.id) })} /><span>{new Date(option.start).toLocaleString()} · {option.votes} {option.votes === 1 ? 'vote' : 'votes'}</span></label>)}</fieldset><button className="btn btn-primary">Save availability</button>{status && <p>{status}</p>}</form>}</>}</main></div>;
}
