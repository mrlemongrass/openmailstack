...
export function SchedulerToolsPanel({ state, onChanged }: { state: SchedulerState; onChanged: () => Promise<void> | void }) {
  ...
  const [finalizing, setFinalizing] = useState<SchedulerState['polls'][number] | null>(null);
  useSchedulerDraft(Boolean(booking.start || booking.bookerName || booking.bookerEmail || poll.title || poll.starts.some(Boolean)), busy);
  const begin = () => { if (mutation.current) return false; mutation.current = true; setBusy(true); return true; };
  const finish = () => { mutation.current = false; setBusy(false); };
  ...
  const book = async (event: React.FormEvent) => {
    event.preventDefault(); if (!begin()) return; setStatus('Booking…');
    try {
      const payload = { ...booking, eventTypeId: eventId, start: localIso(booking.start), bookerTimeZone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC' };
      const identity = JSON.stringify(payload);
      if (bookingAttempt.current?.payload !== identity) bookingAttempt.current = { payload: identity, key: crypto.randomUUID() };
      await bookSchedulerOnBehalf({ ...payload, idempotencyKey: bookingAttempt.current.key });
    ...
    } catch (error) { setStatus(error instanceof Error ? error.message : 'Unable to create booking; your entries are retained'); }
    finally { finish(); }
  };
  const createPoll = async (event: React.FormEvent) => {
    event.preventDefault(); if (!begin()) return; setStatus('Creating poll…');
    try {
      const result = await createSchedulerPoll({ eventTypeId: eventId, title: poll.title, starts: poll.starts.map(localIso) });
      setNewPollUrl(result.url); setPoll({ title: '', starts: ['', ''] }); setStatus('Poll created. Copy this link now; its secret token is shown once.'); await onChanged();
    } catch (error) { setStatus((error instanceof Error ? error.message : 'Unable to create poll') + '. Refresh the poll list to check whether it was created before trying again.'); }
    finally { finish(); }
  };
  ...
  const importFile = async (file?: File) => {
    ...
    try {
      ...
      await onChanged();
    } catch (error) { setStatus((error instanceof Error ? error.message : 'Unable to import file') + '. Refresh event types to check for partial imports before retrying.'); }
    finally { finish(); }
  };
  const finalize = async () => {
    if (!finalizing || !begin()) return;
    try {
      await finalizeSchedulerPoll(finalizing.id, [...finalizing.options].sort((a, b) => b.votes - a.votes)[0].id);
      setStatus('Poll finalized'); await onChanged();
    } catch (error) { setStatus((error instanceof Error ? error.message : 'Unable to finalize poll') + '. Refresh to check the saved state before retrying.'); }
    finally { finish(); setFinalizing(null); }
  };
  ...
  return <div className="scheduler-tools">
    <ConfirmDialog open={Boolean(finalizing)} title="Finalize this poll?" message={finalizing ? `${finalizing.title}: choose the option with the most votes and create its booking. This closes the poll.` : ''} busy={busy} confirmLabel="Finalize poll" onConfirm={() => void finalize()} onCancel={() => setFinalizing(null)} />
    <fieldset disabled={busy} style={{ display: 'contents' }}>
    <div className="scheduler-section-title"><div><h1>Tools</h1><p>Delegated booking, polls, waitlists, sharing, and migration</p></div><span role="status">{status}</span><button type="button" className="btn btn-secondary" onClick={() => void onChanged()}>Refresh saved results</button></div>
