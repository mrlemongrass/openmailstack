...
function EventEditor({ event, calendars, defaultAvailability, onClose, onSaved }: {
  ...
  const [form, setForm] = useState<Partial<SchedulerEventType>>({
    ...
    activeBookingLimit: event?.activeBookingLimit ?? null,
    guestAllowList: event?.guestAllowList || [],
    ...
    <div className="scheduler-modal-backdrop">
      <form className="scheduler-modal" role="dialog" aria-modal="true" aria-labelledby="event-editor-title" onSubmit={submit}>
        ...
        {section === 'limits' && <div className="scheduler-form-grid scheduler-editor-section">
          ...
          <label>Capacity<input type="number" min={1} max={100} value={form.capacity} onChange={e => { const capacity = Number(e.target.value); setForm({ ...form, capacity, maxAdditionalGuests: Math.min(form.maxAdditionalGuests || 0, Math.max(capacity - 1, 0)) }); }} /><small>Total seats available at each time</small></label>
