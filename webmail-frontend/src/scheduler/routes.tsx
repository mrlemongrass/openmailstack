...
function EventEditor({ event, calendars, defaultAvailability, onClose, onSaved }: {
  ...
  const [form, setForm] = useState<Partial<SchedulerEventType>>({
    ...
    cancellationCutoffMinutes: event?.cancellationCutoffMinutes ?? null,
    rescheduleCutoffMinutes: event?.rescheduleCutoffMinutes ?? null,
    requireCancellationReason: event?.requireCancellationReason ?? false,
    requireRescheduleReason: event?.requireRescheduleReason ?? false,
    ...
    <div className="scheduler-modal-backdrop" onMouseDown={onClose}>
      <form className="scheduler-modal" role="dialog" aria-modal="true" aria-labelledby="event-editor-title" onSubmit={submit} onMouseDown={eventMouse => eventMouse.stopPropagation()}>
        ...
        {section === 'limits' && <div className="scheduler-form-grid scheduler-editor-section">
          ...
          <label>Cancellation cutoff<input type="number" min={0} max={525600} step={15} value={form.cancellationCutoffMinutes ?? ''} onChange={e => setForm({ ...form, cancellationCutoffMinutes: e.target.value === '' ? null : Number(e.target.value) })} placeholder="No cutoff" /><small>Minutes before the meeting; blank allows cancellation anytime.</small></label>
          <label>Reschedule cutoff<input type="number" min={0} max={525600} step={15} value={form.rescheduleCutoffMinutes ?? ''} onChange={e => setForm({ ...form, rescheduleCutoffMinutes: e.target.value === '' ? null : Number(e.target.value) })} placeholder="No cutoff" /><small>Minutes before the meeting; blank allows anytime and zero allows changes until start.</small></label>
          <label className="scheduler-publish"><input type="checkbox" checked={form.requireCancellationReason === true} onChange={e => setForm({ ...form, requireCancellationReason: e.target.checked })} /><span>Require a cancellation reason<small>Stored privately with the booking for the owner.</small></span></label>
...
export function SchedulerRoutes() {
  ...
  return <div className="scheduler-app">
    ...
    {selectedBooking && <div className="scheduler-modal-backdrop" onMouseDown={() => setSelectedBooking(null)}><section className="scheduler-booking-detail" onMouseDown={event => event.stopPropagation()}><header><div><h2>{selectedBooking.event.title}</h2><p>{selectedBooking.status}</p></div><button className="icon-button" onClick={() => setSelectedBooking(null)} aria-label="Close"><X size={18} /></button></header><dl><div><dt>Guest</dt><dd>{selectedBooking.bookerName}<span>{selectedBooking.bookerEmail}</span></dd></div><div><dt>When</dt><dd>{new Date(selectedBooking.start).toLocaleString()}<span>{selectedBooking.event.durationMinutes} minutes</span></dd></div><div><dt>Location</dt><dd>{selectedBooking.event.locationLabel || 'Not specified'}</dd></div>{selectedBooking.bookerNotes && <div><dt>Notes</dt><dd>{selectedBooking.bookerNotes}</dd></div>}{selectedBooking.cancellationReason && <div><dt>Cancellation reason</dt><dd>{selectedBooking.cancellationReason}</dd></div>}{selectedBooking.rescheduleReason && <div><dt>Reschedule reason</dt><dd>{selectedBooking.rescheduleReason}</dd></div>}{(selectedBooking.bookingAnswers || []).map(answer => <div key={answer.questionId}><dt>{answer.label}</dt><dd>{answer.value}</dd></div>)}</dl></section></div>}
  </div>;
}
